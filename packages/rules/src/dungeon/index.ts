...
function unit(species: string, id: string, hp?: number): Unit {
  ...
  return {
    ...
    max: hp ?? t.hp,
    hp: hp ?? t.hp,
    uses: t.moves.map((_, i) => (enemy ? -1 : i === 3 ? 1 : 3)),
    snared: 0,
    ward: false,
    charge: null,
    recovery: 0,
...
export function legalMoves(u: Unit): number[] {
  ...
  const moves = u.moves
    ...
    .filter(
      (i) =>
        (u.enemy || u.uses[i] > 0) &&
        !(u.snared && u.moves[i].range === "melee") &&
        !(u.recovery && u.moves[i].kind === "charge")
    );
  // Explicit exhaustion-only option; never substitutes for a failed signature.
  if (
    !u.enemy &&
    !u.snared &&
    !u.moves.some((m, i) => m.kind === "hit" && u.uses[i] > 0)
  )
    moves.push(-1);
  return moves;
}
...
function prepare(s: Run) {
  ...
  for (const u of standing(s.enemies)) {
    ...
    else if (u.species === "shield" && available.length > 1)
      move = random(s) < 1 / 3 ? 1 : 0;
...
function enter(s: Run) {
  ...
  s.enemies = ROOMS[s.room].enemies.map((row) =>
    unit(String(row[0]), String(row[1]), Number(row[2]))
  );
...
export function createRun(seed = 1): Run {
  const s: Run = {
    ...
    room: 0,
    round: 1,
    team: ["graviclaw", "avilily", "crystorn", "hippochamp"].map((key, i) =>
      unit(key, ["G", "A", "C", "H"][i])
    ),
    enemies: [],
    orders: {},
  ...
  for (const u of sequence) {
    ...
    if (q.move === -2) emit(`${u.name} cannot act while restrained.`);
      ...
      if (restrained && m.range === "melee") {
        ...
        emit(
          `${u.name}'s ${m.name} is stopped by restraint.${
            release
        ...
        if (target) {
          ...
          if (m.kind === "ward") {
            ...
            emit(
              `${u.name} uses ${m.name} on ${target.name} (${
                target.id
...
export function command(previous: Run, action: Command): Run {
  ...
  if (action.kind === "revive") {
    ...
    s.room++;
    if (s.room === 3) {
      for (const u of standing(s.team)) u.hp = Math.min(u.max, u.hp + 10);
      s.log.push("Recovery station: +10 HP to standing squad members.");
    }
    enter(s);
  }
  return s;
