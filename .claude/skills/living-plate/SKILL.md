...
6. **Bake, then verify on the page.** After every export, `node scripts/plates/bake-plate.cjs <era>` (the site plays the baked `live.html`, and the bake fails if it does not match the export). Dev server on port 3012, `node scripts/plates/snap-home-plate.cjs`, both widths and reduced motion, no console errors. Then PR from a fresh branch off main with auto-merge.
