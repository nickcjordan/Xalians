...
The complete 32-species v5 roster is available. The current frozen canonical v5 release is `generation-0.8.0-1` on schema 5.1.0; earlier archives remain replayable. Existing `docs/species-templates/<key>.json` and the legacy public generator remain v4, while Powerworks and Reclamation consume the frozen v5 roster. Author future v5 species under `docs/species-templates/v5/`; a substantive change needs a new release ID and separately scoped game adoption. Schema 5.1 requires `massKg` plus at least one applicable overall `heightCm`, `lengthCm` or `widthCm`; it does not add structured appendage measurements. Do not feed v5 definitions into the v4 bundler. If explicitly working on the deployed game format, inspect its actual schema and validator instead of copying fields from the redesign. Do not interrupt another agent's species work.
