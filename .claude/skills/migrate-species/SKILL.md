...
3. Author an ordinary `mechanisms` entry only for what the derivation tables cannot say: an effect type, status, direction, delivery mode or area the rows do not produce. Anything the tables already produce for the same instrument, element, delivery mode, targeting, area and effect types is redundant and is left to derivation. Author over catalog values with the supported nested relationships. No complete-move whitelist, extra whole-ability template catalog, arbitrary predicates, or independent trait rolls. All inherent effects remain present; status application likelihood can vary.
...
6. Run `npm run check:creature-model -- docs/species-templates/v5/<key>.json`. This invokes the shared compiler, proves four distinct actions can be selected, and prints the distinct act count with a per-instrument breakdown and the exclusions in force. Read that breakdown, then add an `acts.exclude` entry only where the lore, the audit's own exclusion decisions or the physiology contradicts a derived act, with a one-line reason written in the audit. Do not exclude on taste or to keep a species focused. Where a removed mechanism carried a deliberate band far from the derived default, put that band into `acts.output`. Resolve failures in authoring permissions, not by truncating, padding or adding generation retries.
...
8. Write `<key>.ability-audit.md` beside the staged template. Record included/excluded mechanism families, source evidence, retired powers and genuine remaining questions, plus the declared channels and conduits with their evidence, the mechanisms removed as redundant and those kept, and every `acts.exclude` entry with its reason. A schema pass alone does not demonstrate creative completeness.
