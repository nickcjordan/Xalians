...
6a. **Misspellings are never carried (Nick, 2026-09-02).** If a source description contains an unambiguous misspelling (temperment, pedals for petals, levatating), record it in the walkthrough and in your final report so the orchestrator corrects `species.json` upstream; the record then carries and quotes the corrected text. Never reproduce a misspelling in a record on the grounds of verbatim carry.
...
```jsonc
{
  ...
  "lore": {
    ...
    "appearance": ["...", "..."],           // 3 to 8 defining presentation qualities, one per entry (lore section of 5)
    "origin": "...",                        // five required short fields (lore section of 5): who made it and what for
    "habitat": "...",                       // where it lives now
    "feeding": "...",                       // how it eats
    "behavior": "...",                      // how it goes about its business
    "company": "..."                        // alone, pairs, crews, swarms; all five required, never omitted
  },
  "physiology": {
...
POOL SHAPE (Nick, 2026-09-08, read this first): required traits at 100 are one at minimum and two normally (three only as a justified special case), chosen as the traits that define the species in its own lore (body facts from anatomy, covering or corporeality first, then the behavior the description or signature is built around); every other evidenced trait is rolled, 5 to 60 each, with the rolled percents summing to exactly 100; at most six entries in the pool; a trait never restates a record field (the senses list never makes perceptive required); a planet-wide adaptation is required only when the description makes it the point of the creature; an output priority alone never brings a trait in without a fauna observation or species sentence covering the form; legacy statRatings of high rank a rolled trait first and never set a percent. The validator warns outside this shape. Batch rulings from the 2026-09-10 re-run (orchestrator, overridable by Nick): (1) a ratified `company` line is source for `pack-bonded` or `solitary` but sets at most the top rolled share, never a required slot, because a social fact is not a body fact and the required slots belong to anatomy, covering, corporeality and the act the description is built around; (2) `pack-bonded` needs a sentence showing joint action, not co-location (Terragoyle standing in a line with others along the rim is territorial, not pack-bonded); (3) the six-entry cap does not flex: when a newly sourced trait enters a full pool, the weakest-evidenced entry leaves, and the walkthrough says which and why; (4) the 2 to 8 percent band for `foresighted` and `telekinetic` is the default for a species whose sources merely suggest the trait; a species whose description is built on the act carries it at what the evidence supports (Terragoyle 100, Graviclaw rolled at 22).
...
- `appearance` (Nick, 2026-09-09, replaces the prose `body` field): a list of 3 to 8 defining presentation qualities, one quality per entry, each a short plain-words phrase (never a registry key, never a full sentence chain). Order: overall form first (what it resembles, how it stands, its size relative to a person or to its own height), then covering, then the defining structures, then any sense organ large enough to be a feature. Sizes are relative words only ("tall as a person", "heavy for its height", "dog-sized"); the measurements already live in `physiology.size` and never repeat here. Sources are the teaser, the record fields, and the art for major visible features: large rabbit-like ears atop the head, very large eyes, a shaggy coat, a crest, a coiled tail. The art is never a source for how a feature is shaped, angled, notched or counted (no tooth shape, no pupil shape, no "three or four toes", no tuft placement, no pose). Test: a second artist drawing the species from this list would produce the same creature, and could still draw it their own way.
- **Entry rules (Nick, 2026-09-09, from his corrections on the first list pass).** (1) One concept per entry: "Black chitin shell" and "Heavy for its height" are two entries, never one. (2) Physical presentation only: an eye is a feature, "nearly blind" is a capability and does not belong (a creature can have huge eyes and be blind). (3) Never list anatomy the creature lacks; say "without limbs" only when the absence is the form itself, never "no legs, no wings, no jaws". (4) Never state a default: every hide is unarmored unless armor is named, so "unarmored" is noise. (5) No definitive claims that fence future lore: never "the largest thing on Endessa", never "the only". (6) Each entry is a readable phrase, not a clause torn from a sentence ("Surrounded by a swarm of summoned flying familiars", not "The swarm of familiars it summons"). (7) Look at the art yourself before writing; the walkthrough art readings have been wrong about major features (a pair of ram horns read as one horn; a raised forelimb read as a crest; an inverted squid read as a trunk with shoots), and the orchestrator checks every list against the PNG.
- **Five short fields (Nick, 2026-09-10, replace `habits` and `biomeNiche`).** `origin` (who generated it and what for), `habitat` (where it lives now), `feeding` (how it eats), `behavior` (how it goes about its business, the signature act in plain words), `company` (alone, pairs, crews, swarms). Each is one to three sentences or a short phrase in ordinary language, written the way a person would say it. A phrase is fine; never pad a phrase into a sentence and never strip words from one for brevity. All five are required (Nick, 2026-09-10): a consumer that reads a field must never find it missing. Where the canon is silent the field carries the plainest reading the record supports (the diet key fixes feeding; a lost purpose is written as "No record says what it was made for"; company is never silently defaulted to solitary, and every authored company line is flagged for Nick). Never pad a field to fill it. Every claim is anchored to the teaser, a planet-history sentence, a Generator report line or a record field. Planet history stays in the planet records: a field names the world only where the creature itself is involved (Hypnopets pressed into Kozrak's crowd control, yes; the Grimedites watching for APEX, no). No sign-off lines. No environmental-tolerance sentences written to justify a trait; trait evidence comes from the record and the report. No template phrases ("its ground" is struck). Why: the habits paragraph was a checklist forced into one paragraph, which produced compressed sentences, a repeated template phrase, planet exposition and sign-off lines; the fields give the structure its own shape.
- Overlap with the teaser (lever set 2026-09-09 after the first roster pass): appearance and the short fields never copy a teaser sentence, but each MAY restate the defining physical or behavioral fact from its own angle when leaving it out would make the field describe the world acting on the creature instead of the creature acting (a Hypnopet's behavior says what the horn does to a patient; a Yetimoth's behavior says how it walls a corridor). The Generator report and the planet history are the sources that reliably add what the teaser does not.
...
For new abilities apply section 5.8. Historical reservation warnings retain the former matrix as an audit aid; a source-supported new delivery/effect combination need not fit it.

### 5.8 Actions, passives, signature, and standard action pool (schema 4)

Read [the current ability model](../../../docs/design/xalian-ability-model.md). It supersedes the earlier repertoire/inherent/prominence proposal. Earlier ledgers and walkthroughs remain historical evidence.

- Exactly one signature reference resolves to an entry in actions or passives and preserves the species' essential distinctive process. Multiple connected effects are allowed; unrelated guaranteed powers are not bundled together. Guaranteed baseline functioning belongs in physiology, traits, or other species properties.
- Standard actions are randomly drawn from actionPool. Guaranteed actions contain only the signature; passives are guaranteed separately. Each weighted set must consist entirely of mutually compatible, independently usable options. Generation selects one set and draws within it; incompatible alternatives belong in different sets. The count band controls the draw size.
- Each option must work with guaranteed properties/signature; it cannot depend on another random ability, an optional trait, or a rolled secondary affinity. Every set needs enough primary-medium options to satisfy its maximum count. All permitted subsets and media must be coherent by authoring, not by evaluating generated individuals.
- Separate instrument, medium, activation, timing, delivery, spatial, targeting, and typed effects. Deliberate actions include chosen reactions; automatic ongoing or triggered processes belong in passives. Use operation discrete/ongoing and at most one contact/incoming-harm/ally-distress trigger. The instigator recipient means the participant causing that event. Persistence alone does not distinguish a body property from an ability. Do not duplicate a mechanism across fields.
- Author a stable key, name, instrument, medium, activation, delivery, spatial, targeting, effects, intensity band, and description. Actions and discrete passives require preparation/recovery timing; ongoing passives omit it. Range is contact/short/medium/long; omit it for self-only effects and creature-centered radial areas. Optional area declares shape, extent, anchor; spatial declares selectivity. Effects each declare recipient, emphasis (exactly one primary), onset, persistence, likelihood, and a brief/prolonged duration only if lingering. Effect compatibility may narrow targeting. Sustained effects require ongoing operation. Status effects use the shared 26-status catalog and explicit removable methods; removal effects provide methods. Resistant requires exposure; stimulated requires function. No implicit cures from elements, universal cleanse, aftermath chain, nested conditions, or optional suppression. The complete current enum definitions and examples are in the linked model.
- Pure support is valid. Do not add damage to satisfy every game or require universal solo viability. Standard options still require anatomical, elemental, or supernatural source support.
- Restoration is not resurrection or a cure for the Nemesis Plague. Compatibility is not universal. Keep source predicates, canon voice, existing reserved names, and collision checks. Game costs, turns, HP, and stacking do not belong in canonical facts.
- Catalog families supply names, not meanings. Check every actual pool option's instrument/media/pattern name family. Do not use the historical matrices as automatic generation permissions.
- If a needed pattern falls outside this skill's file scope, describe it for the coordinating author. Never reference an absent pattern or hide unsupported dependencies in prose.
...
2. **Lore fields.** Copy the species.json description into `lore.description` verbatim, whatever its length. Write `lore.appearance` and the five short fields per the lore section of 5 (registers, sources, omission rule). Record their review status in `docs/species-templates/lore-status.json`, never in the record.
...
9. **Trait pool.** Sources for the pool (2026-09-10): the teaser, the art, the planet record, and the species' ratified `lore.appearance` and five fields (ratified for all thirty as of 2026-09-10; `docs/species-templates/lore-status.json` says which); a ratified `company` field is source for `pack-bonded` or `solitary`, a ratified `behavior` or `feeding` field for the behavior traits, and `appearance` for body facts, each quoted like any other source. Every trait the species can carry, each with its independent percent and a one-line reason; name in the walkthrough any trait the body could plausibly carry that you left out, with the reason; body-demanded and environment-demanded traits at or near 100; rare traits in single digits; never both exclusion partners at 100. State the expected trait count (sum of percents divided by 100) in the walkthrough.
10. **Signature and action pool.** Identify the essential signature process and permitted standard options (quote their evidence). Apply section 5.8 and the current ability model. Classify instrument, medium, delivery, targeting, activation, and effects against the FULL description (the pilot lesson, ratified by Nick: the instrument is the body part where the effect TERMINATES on the target, not the physics that produces it; if prey is drawn into the creature's grip, the instrument is the gripping part, not `mind` and not `aura`, even though the pull is gravitational or mental in origin). Before coining a name, search every `consolidated-*.md` and `neutral-pools.md` for the species name (case-insensitive): a catalog ledger note that reserves or records a signature name for this species is a ratified prior ruling by Nick; use that name unless the source description contradicts it, and say so in the walkthrough. Otherwise coin a grander-register name and run the collision scan: search every `consolidated-*.md` and `neutral-pools.md` for the exact name (case-insensitive); rename if found. Write the one-line description in canon voice.
11. **Catalog check through the species lens.** For each explicitly permitted standard-pool instrument, medium, and pattern name-family combo, count the drawable names in the catalog cell plus the neutral pool (respecting instrument tags). Report any combo with fewer than 6 names as a thin-combo finding in the walkthrough. Never pad a cell yourself.
12. **Encyclopedia entry.** Append the entry straight into `docs/encyclopedia/encyclopedia.json`'s `entries` array (keys append-only, never reorder or delete existing entries): `{ "key": "<key>", "title": "<Name>", "category": "xalians", "definition": "...", "related": [] }` in the Encyclopedia register (section 3), one or two sentences, built only from the template and the sources. The orchestrator computes `related` mechanically once the entry is ratified. There is no side file: species records live only in `docs/species-templates/<key>.json` and every encyclopedia entry, species included, lives only in `docs/encyclopedia/encyclopedia.json`.
...
- **After Nick ratifies** (orchestrator, not the migration agent): add the key to `docs/species-templates/RATIFIED.json`, run `node scripts/bundleLore.js` from the repo root (CI fails if the committed bundle in `packages/content/json` is stale, and runs `docs/species-templates/tools/validate-all.js` over every ratified key), and confirm the species page at `/encyclopedia/species/<key>` renders the template (the page resolves every registry key through `docs/species-templates/registries.json`; a key missing there prints raw, so add its row first). Unratified templates never ship: the bundler reads only the manifest.

## 7a. Brand-new species

A species that does not yet exist in `species.json` starts with `node docs/species-templates/tools/new-species.js --key <key> --name <Name> --element <Element> --planet <Planet> --height "<in> in / <cm> cm" --weight "<lbs> lbs / <kg> kg" --description "<Nick's teaser>"`, which appends the legacy entry with the next id, adds the pending row to `lore-status.json`, and prints the remaining steps; the checklist is `docs/species-templates/NEW-SPECIES.md`. The art at `docs/species-templates/art/<key>.png` must exist before the migration agent runs. Then this skill runs exactly as for a migration.
...
- The Encyclopedia page (`docs/design/xalian-encyclopedia-page.md`): the consumer of every template. Its data layer (`apps/web/src/lore/species.js`) builds a SpeciesView from the template shape in section 4, so a template field renamed here must be renamed there.
