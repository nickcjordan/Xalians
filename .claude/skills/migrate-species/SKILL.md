...
2. **Lore fields.** Copy the species.json description into `lore.description` verbatim, whatever its length. Write `lore.appearance` and `lore.habits` per the lore section of 5 (registers, sources, word ranges). Record their review status in `docs/species-templates/lore-status.json`, never in the record.
