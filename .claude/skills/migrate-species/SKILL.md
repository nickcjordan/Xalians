...
1. **Location.** All work happens in the creature worktree the orchestrator names (a checkout of this repo on a branch off `main`; never the checkout another session owns). Every path in this skill is relative to that worktree root. Read inputs from `packages/content/json/` (the single content source since the 2026-09-10 workspace split; `lambda/src/json` and `my-app/src/json` no longer exist). Write outputs only to `docs/species-templates/`. The validator script lives at `docs/species-templates/tools/validate-template.js` and is run from the worktree root. Never write anywhere else, never write into any plans folder, never write into `docs/species-templates/overrides/` or `validation-log/`, never run git commands (Fable commits).
