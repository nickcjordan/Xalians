...
- **Encyclopedia entry register**: one or two sentences, encyclopedic and definitional, leading with the category noun, cross-referencing canon terms by name, no flourish, no ellipsis.
...
- Size: realistic absolutes in cm and kg with deliberately dramatic spread across species; the legacy numbers are only the relative gauge; mass serves the fantasy (an immovable creature is heavy).
...
Limbs: `claws` · `talons` (raptorial grip-and-pierce) · `fists` (blunt striking hands) · `hooves` · `pincers` (opposing grip) · `blades` (limb ending in a cutting edge) · `spurs` (limb spikes) · `wings` (strike, gust, shroud as well as flight).
...
10. **Signature ability.** Identify the lore-defining act from the description (quote it). Classify instrument, action, medium against the FULL description (the pilot lesson: when the description delivers an effect through a named body part, the instrument is that part, not a channel like `mind`, even if the power sounds mental). Coin a grander-register name and run the collision scan: search every `consolidated-*.md` and `neutral-pools.md` for the exact name (case-insensitive); rename if found. Write the one-line description in canon voice.
