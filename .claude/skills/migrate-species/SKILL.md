...
Surface and armor: `hide` (body surface used defensively: bare, leathery, or scaled skin; a species declares ONE surface key, so a shelled or plated body is `shell` and never also `hide`, whatever the covering enum says) · `shell` (rigid enclosing casing permitting withdrawal; an exoskeleton or carapace is `shell`) · `spines` (rigid projections: quills, barbs, thorns, crystal growths).
