...
The current v5 roster is available. The species-independent engine is `generation-0.9.0-1`; record schema is 5.2.0, while authoring templates remain on schema 5.1.0. Earlier combined archives remain replayable. Existing `docs/species-templates/<key>.json` and the legacy public generator remain v4, while Powerworks and Reclamation consume the v5 roster. Author future v5 species under `docs/species-templates/v5/`. After ratification run `npm run sync:species` and `npm run check:species` to publish a species content revision. A creature-only change fitting the current schema and shared vocabulary does not change generator code, version, or archive. Game adoption remains separately scoped. Schema 5.1 templates require `massKg` plus at least one applicable overall `heightCm`, `lengthCm` or `widthCm`; they do not add structured appendage measurements. Do not feed v5 definitions into the v4 bundler. If explicitly working on the deployed game format, inspect its actual schema and validator instead of copying fields from the redesign. Do not interrupt another agent's species work.
