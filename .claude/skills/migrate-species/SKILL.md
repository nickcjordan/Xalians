...
```jsonc
{
  ...
  "traits": {
    "pool": { "traitKey": percent }             // each trait rolled INDEPENDENTLY at its own percent (1 to 100); 100 = every individual; omit a trait rather than writing 0
  },
...
9. **Trait pool.** Every trait with its independent percent, each justified; body-demanded and environment-demanded traits at or near 100; rare traits in single digits; never both exclusion partners at 100. State the expected trait count (sum of percents divided by 100) in the walkthrough.
