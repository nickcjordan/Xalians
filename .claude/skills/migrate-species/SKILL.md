...
- `communication` (array): `vocal | vibration | display | chemical | telepathic`; `[]` = mute. If neither source describes any call, signal, display, scent, or telepathic behavior, the value is `[]` and the walkthrough lists the field as authored-absent; never invent a channel from body plausibility (v2.2 rule, after a run authored `vibration, display` with no source sentence).
...
- Temperature: a sustained-normal-activity band in Celsius. It must lie inside the planet `data` block's `Temperature Low` to `Temperature High` range; a narrower sub-band is expected (a wetland lurker does not tolerate the planet's hottest extreme) and is justified from the history and the body. Extending past the planet range in either direction requires a quoted source sentence; without one it is a validation failure (v2.2 rule, after a run banded 34 degrees below the planet's stated low).
