...
```jsonc
{
  ...
  "lore": {
    ...
    "body": "...",                          // physical description, 25 to 120 words (lore section of 5)
...
- `body`: what it looks like and how it is built, 25 to 120 words, one paragraph, present tense, physical only: the parts in plain words (never registry keys), the covering, the size feel, how it moves, what it senses with. **Sources are the teaser and the record fields only** (anatomy, covering, composition, size, capability and sense bands). The art is not a source for the body: the art reading in the walkthrough keeps what the drawing shows, and none of it enters the body unless the teaser or a record field also says it. No purpose, no events, no planet history.
- **Defining features only (Nick, 2026-09-09).** The body describes the creature, not the drawing. A detail belongs only if a second artist drawing the species from the record would have to keep it: the body plan and limb arrangement, the structures the record lists as anatomy (the crest, the blade-trunk, the pincer, the horn, the auger tail), the covering and composition, the size feel, the locomotion it is built for, and the sense it leads with. Leave out what is merely how this one image was rendered: tooth shape, pupil shape, how many eyes or nostrils or digits or tufts were drawn, banding counts, where the tufts sit, the pose in the one drawing, and any phrasing about how something is "drawn" or "reads". The art reading in the walkthrough keeps those observations; the body does not. Test: if Nick would not object to the next artist changing it, it is not a body fact.
