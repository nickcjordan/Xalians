...
1. `packages/content/json/planets.json` histories (read with `node -e`, per planet, per paragraph).
...
1. **Run the structural validator first.** `npm test -- --run -t tour` from `apps/web/` (`src/lore/__tests__/lore.tour.test.js`): sources resolve, entries exist, no em-dashes, no four-digit numbers, word bands, banned words. Fix failures before reading further.
...
5. **Bundle** (`node scripts/bundleLore.js`), commit, and report to Nick: texts checked, claim counts per class, what was changed, in prose. Do not ask him to verify.
