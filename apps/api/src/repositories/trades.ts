...
const PROPOSER_INDEX = 'byProposer';
const RECIPIENT_INDEX = 'byRecipient';
const LIST_LIMIT_PER_ROLE = 100;
...
// Each side of a trade has its own GSI because a DynamoDB item cannot contribute two
// values to one partition key. Query both roles in parallel, then present one newest-first
// activity stream. A trade can never appear in both results because self-trades are
// rejected when created.
export async function listTradesForUser(userId: string): Promise<TradeOffer[]> {
  try {
    const [proposed, received] = await Promise.all([
      ddb.send(
        new QueryCommand({
          TableName: TABLE_NAME,
          IndexName: PROPOSER_INDEX,
          KeyConditionExpression: 'proposerId = :userId',
          ExpressionAttributeValues: { ':userId': userId },
          ScanIndexForward: false,
          Limit: LIST_LIMIT_PER_ROLE,
        })
      ),
      ddb.send(
        new QueryCommand({
    ...
    return [...(proposed.Items ?? []), ...(received.Items ?? [])]
      .map((item) => TradeOfferSchema.parse(item))
      .sort((a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt));
  } catch (err) {
    log.error('listTradesForUser failed', {
      userId,
      errorName: err instanceof Error ? err.name : typeof err,
    });
    throw err;
...
export async function acceptTrade(trade: TradeOffer, respondedAt: string): Promise<TradeOffer> {
  const ownershipUpdates = [
    ...trade.offeredXalianIds.map((xalianId) => ({
      xalianId,
      from: trade.proposerId,
      to: trade.recipientId,
    })),
    ...trade.requestedXalianIds.map((xalianId) => ({
      xalianId,
      from: trade.recipientId,
      to: trade.proposerId,
    })),
  ];
