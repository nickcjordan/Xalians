...
test.each([[0, null], [3, null], [5, 'Last energy'], [6, "Can't act"]])('energy load %i shows current condition without unrelated role instructions', (load, warning) => {
  ...
  expect(screen.queryByText('Weakened')).toBeNull();
});
...
test('recovery removes the low-reserve warning and imminent instability is explicit', () => {
  ...
  expect(screen.queryByText('Last energy')).toBeNull();
