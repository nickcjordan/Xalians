  ...
  return <section className="lr-lead-options" aria-label="Who leads?">
    <div className="lr-lead-options-list" aria-label="Compare crossing leads">
      {plans.map(plan => { const cost = comparisonCosts(plan, companion); const identity = methodIdentity(plan.method); const leadCost = Math.max(0, cost.energy - plan.baseSupportStrain); return <button key={plan.lead.id} type="button" aria-pressed={plan.lead.id === selectedId} onClick={() => onSelect(plan.lead.id)}>
        ...
        <span className="lr-lead-option-cost" aria-label={`${cost.energy} known energy cost, ${cost.stability} known stability cost`}><span><Zap size={18} />{cost.energy === 0 ? 'No known energy cost' : `Spend ${cost.energy} energy`}</span>{cost.saved > 0 && <small className="lr-lead-ally-saving">{companion.creature?.species || 'Ally'} saves 1 · uses its one help</small>}{cost.stability > 0 && <span><Building2 size={18} />Lose {cost.stability} stability</span>}<small><Hourglass size={16} />{identity.limited ? 'Ability unavailable after crossing' : 'Keep one-use abilities'}</small>{plan.baseSupportStrain > 0 && <small className="lr-lead-shared-cost">Energy shared: {plan.lead.species} {leadCost} · {plan.support.species} {plan.baseSupportStrain}</small>}</span>
