...
export function scoutBeats(action) {
  ...
  return [
    ...
    { kind: 'observe', title: found ? action.result.relay ? 'The scout finds a clue' : 'Something ahead' : 'Searching ahead', icon: 'bi-eye-fill', text: found ? action.result.relay ? clues || `The scout picks out ${found === 1 ? 'a hidden danger' : `${found} hidden dangers`} along the crossing.` : `Something catches ${action.scout.species}'s attention along the crossing. The waiting crew has not heard what it found.` : `Nothing gives ${action.scout.species} a clear warning. The way may still hold danger.` },
