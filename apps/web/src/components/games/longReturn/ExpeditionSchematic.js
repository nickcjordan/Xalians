...
export default function ExpeditionSchematic({ scene, crew = [], scout, helperId, position = { crew: 'entry' }, routeId, revealedIds = [], native, nativeState, companion, allyWithScout = false, runFlags = [], compact = false, localOnly = false, readingRecord = false, decisionInset = false, preview = false, reserves, onNextDecision, className = '' }) {
  ...
  return <figure data-tier="immersive" data-expedition-map={decisionInset ? undefined : true} data-route-schematic={decisionInset ? true : undefined} data-map-scene={scene.id} data-map-local={localOnly ? 'true' : undefined} data-map-record={readingRecord ? 'true' : undefined} data-preview-route={preview ? routeId : undefined} data-crew-position={position.crew} className={`m-0 min-w-0 border-y border-edge bg-s0 text-ink ${className}`}>
    ...
    {!localOnly && !decisionInset && <div data-site-overview className="flex justify-between px-4 pt-1 text-small text-ink-2"><span>{scene.trackLabel}</span><span>{sceneIndex < 4 || (sceneIndex === 4 && position.crew !== 'exit') ? '◎ Recover the Index' : '◎ Index reached'}</span></div>}
    ...
    {readingRecord && <div data-map-key className="flex flex-wrap gap-x-4 gap-y-1 px-4 pb-3 text-small text-ink-2">{tokens.map(({member,number,place}) => <span key={member.id}><b className="text-viable">{number}</b> {member.species}{place === 'survey' ? ' · ahead' : ''}</span>)}{contact && <span><b className="text-caution">◇</b> {contact.species}{nativeState === 'bypassed' ? ' · Still trapped' : ''}</span>}{ally && <span><b className="text-viable">◆</b> {ally.species} · ally</span>}</div>}
    {!readingRecord && !localOnly && !decisionInset && <div className="flex justify-between gap-3 px-4 pt-2 text-small text-ink-2 md:hidden" aria-label="Approaches on the diagram">{scene.routes.map((route, index) => <span key={route.id} data-map-route-caption={route.id} className={index ? 'text-right' : ''}>{changeLabel(changes[index], route)}</span>)}</div>}
    {reserves && <ExpeditionReserves crew={crew} strain={reserves.strain} pressure={reserves.pressure} />}
    {!decisionInset && !readingRecord && (!reserves || contact || ally || knownHazards.length > 0) && <div className="flex flex-wrap gap-x-4 gap-y-1 px-4 pb-3 text-small text-ink-2">{!reserves && tokens.map(({member, number}) => <span key={member.id}><b className="text-viable">{number}</b> {member.species}</span>)}{contact && <span>◇ {contact.species}{nativeState === 'bypassed' ? ' · Still trapped' : ''}</span>}{ally && <span>◆ {ally.species} · Ally</span>}{knownHazards.length > 0 && <span className="text-caution">! Known danger</span>}{!reserves && <span data-site-next className="ml-auto">→ {MISSION.scenes[sceneIndex + 1]?.trackLabel || 'Extraction'}</span>}</div>}
    {!localOnly && !decisionInset && flags.length > 0 && <div data-site-overview className="flex flex-wrap gap-3 border-t border-edge px-4 py-2 text-small text-ink-2" aria-label="Lasting site changes">{flags.map(flag => <span key={flag.id}>↳ {flag.label}</span>)}</div>}
  </figure>;
}
