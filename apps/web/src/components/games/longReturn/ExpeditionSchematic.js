...
export default function ExpeditionSchematic({ scene, crew = [], scout, helperId, position = { crew: 'entry' }, routeId, revealedIds = [], native, nativeState, companion, allyWithScout = false, runFlags = [], compact = false, localOnly = false, readingRecord = false, preview = false, reserves, onNextDecision }) {
  ...
  return <figure data-tier="immersive" data-expedition-map data-map-scene={scene.id} data-map-local={localOnly ? 'true' : undefined} data-map-record={readingRecord ? 'true' : undefined} data-preview-route={preview ? routeId : undefined} data-crew-position={position.crew} className="m-0 min-w-0 border-y border-edge bg-s0 text-ink">
    ...
    <svg viewBox="0 0 600 200" preserveAspectRatio="xMidYMid meet" style={{ width: '100%', height: 'auto', maxHeight: readingRecord ? 280 : compact ? 160 : 240 }} className="block" role="img" aria-label={`${location}. ${place.description} ${tokens.map(token => `${token.member.species}: ${token.place === 'survey' ? 'scouting ahead' : token.place}`).join('. ')}. ${knownHazards.map(hazard => `${hazard.label} known`).join('. ')}.`}>
    ...
    {readingRecord && <div data-map-key className="flex flex-wrap gap-x-4 gap-y-1 px-4 pb-3 text-small text-ink-2"><span><b className="text-viable">●</b> Crew</span>{position.scout && <span><b className="text-viable">⋯</b> Scout ahead</span>}{contact && <span><b className="text-caution">◇</b> Native contact</span>}</div>}
