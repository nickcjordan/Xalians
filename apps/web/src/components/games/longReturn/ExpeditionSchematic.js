...
export default function ExpeditionSchematic({ scene, crew = [], scout, helperId, position = { crew: 'entry' }, routeId, revealedIds = [], native, nativeState, companion, allyWithScout = false, runFlags = [], compact = false, localOnly = false, readingRecord = false, preview = false, reserves, onNextDecision }) {
  ...
  return <figure data-tier="immersive" data-expedition-map data-map-scene={scene.id} data-map-local={localOnly ? 'true' : undefined} data-preview-route={preview ? routeId : undefined} data-crew-position={position.crew} className="m-0 min-w-0 border-y border-edge bg-s0 text-ink">
    ...
    <svg viewBox="0 0 600 200" preserveAspectRatio={readingRecord || localOnly ? 'xMidYMid slice' : 'xMidYMid meet'} style={{ width: '100%', height: 'auto', maxHeight: readingRecord ? undefined : compact ? 160 : 240 }} className={readingRecord || localOnly ? 'block max-md:h-24!' : 'block'} role="img" aria-label={`${location}. ${place.description} ${changes.filter(Boolean).map(change => change.detail).join(' ')} ${tokens.map(token => `${token.member.species}: ${token.place === 'survey' ? 'scouting ahead' : token.place === 'exit' ? place.exit.join(' ') : token.place === 'entry' ? place.entry.join(' ') : token.place}`).join('. ')}. ${knownHazards.map(hazard => `${hazard.label} on ${scene.routes.filter(route => route.hazardIds.includes(hazard.id)).map(route => MAP_ROUTES[route.id] || route.title).join(' and ')}`).join('. ')}. ${scene.routes.map(route => MAP_ROUTES[route.id] || route.title).join(' or ')} lead to ${place.exit.join(' ')}.`}>
