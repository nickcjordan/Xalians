...
export default function ExpeditionSchematic({ scene, crew = [], scout, helperId, position = { crew: 'entry' }, routeId, revealedIds = [], native, nativeState, companion, allyWithScout = false, runFlags = [], compact = false, stageCompact = false, localOnly = false, readingRecord = false, decisionInset = false, preview = false, reserves, onNextDecision, attentionTarget, attentionKey, initialTravel = false, className = '' }) {
  ...
  const location = position.crew === 'exit' ? `Crew at ${place.exit.join(' ')}` : position.scout === 'survey' ? position.encounter ? 'Contact ahead · scout separated' : position.signal ? 'Report received · scout ahead' : 'Scout ahead · crew waiting' : position.crew === 'approach' ? `Crew advancing from ${place.entry.join(' ')}` : position.crew === 'crossing' ? passage?.location || 'Crew on the crossing' : `Crew at ${place.entry.join(' ')}`;
  const tokens = crew.map((member, i) => {
    ...
    if (place === 'crossing' || place === 'approach') point[1] = currentLane;
  ...
  return <figure data-tier="immersive" data-expedition-map={decisionInset ? undefined : true} data-route-schematic={decisionInset ? true : undefined} data-map-stage-compact={stageCompact ? 'true' : undefined} data-map-scene={scene.id} data-map-local={localOnly ? 'true' : undefined} data-map-record={readingRecord ? 'true' : undefined} data-preview-route={preview ? routeId : undefined} data-crew-position={position.crew} className={`m-0 min-w-0 border-y border-edge bg-s0 text-ink ${className}`}>
    <figcaption className={`${stageCompact ? 'lr-stage-caption' : 'lr-expedition-caption'} flex-wrap items-center justify-between gap-2 px-4 text-small font-body`}><strong>{location}</strong>{onNextDecision && <button type="button" className="lr-map-next md:hidden" onClick={onNextDecision}>Scout choices <span aria-hidden="true">↓</span></button>}<span className={`text-ink-2${onNextDecision ? ' hidden md:inline' : ''}`}>{preview ? `Preview: ${MAP_ROUTES[routeId] || scene.routes[routeIndex]?.title}` : 'Site map'}</span></figcaption>
    {!localOnly && !decisionInset && !stageCompact && <div data-site-overview className="lr-overview-row items-center gap-1 px-4 pt-3" aria-label={`Site position: ${scene.trackLabel}, sector ${sceneIndex + 1} of ${MISSION.scenes.length}`}>
    ...
    {!localOnly && !decisionInset && !stageCompact && <div data-site-overview className="lr-overview-row justify-between px-4 pt-1 text-small text-ink-2"><span>{scene.trackLabel}</span><span>{sceneIndex < 4 || (sceneIndex === 4 && position.crew !== 'exit') ? '◎ Recover the Index' : '◎ Index reached'}</span></div>}
    <svg viewBox="0 0 600 200" style={{ width: '100%', height: 'auto', maxHeight: compact ? 160 : 240 }} className="block" role="img" aria-label={`${location}. ${place.description} ${decisionInset ? scene.routes.map((route, index) => `${index ? 'B' : 'A'}: ${route.title}`).join('. ') : ''} ${tokens.map(token => `${token.member.species}: ${token.place === 'survey' ? 'scouting ahead' : token.place}`).join('. ')}. ${knownHazards.map(hazard => `${hazard.label} known`).join('. ')}.`}>
      ...
      {scene.routes.map((route, i) => {
        ...
        return <g key={route.id} data-map-route={route.id}>
          ...
          {decisionInset && <g data-map-choice-tag={i ? 'B' : 'A'} transform={`translate(184 ${y})`}><circle r="16" fill="var(--color-s0)" stroke={selected ? 'var(--color-viable)' : 'var(--color-ink-2)'} strokeWidth="2" /><text textAnchor="middle" y="6" fill={selected ? 'var(--color-viable)' : 'var(--color-ink)'} className="text-body">{i ? 'B' : 'A'}</text></g>}
          ...
          {change && <g data-map-effect={change.flag} transform={`translate(216 ${y - 12})`}><title>{change.detail}</title><circle data-map-effect-symbol cx="13" cy="13" r={stageCompact || decisionInset ? 22 : 16} fill="var(--color-s0)" stroke={changeColor} strokeWidth="2" /><text x="13" y="21" textAnchor="middle" fill={changeColor} className="type-heading">{effectMarks[change.flag] || '•'}</text></g>}
    ...
    {!stageCompact && !decisionInset && !readingRecord && (!reserves || contact || ally || knownHazards.length > 0) && <div className="flex flex-wrap gap-x-4 gap-y-1 px-4 pb-3 text-small text-ink-2">{!reserves && tokens.map(({member, number}) => <span key={member.id}><b className="text-viable">{number}</b> {member.species}</span>)}{contact && <span>◇ {contact.species}{nativeState === 'bypassed' ? ' · Still trapped' : ''}</span>}{ally && <span>◆ {ally.species} · Ally</span>}{knownHazards.length > 0 && <span className="text-caution">! Known danger</span>}{!reserves && <span data-site-next className="ml-auto">→ {MISSION.scenes[sceneIndex + 1]?.trackLabel || 'Extraction'}</span>}</div>}
    {!localOnly && !decisionInset && !stageCompact && flags.length > 0 && <div data-site-overview className="lr-overview-row flex-wrap gap-3 border-t border-edge px-4 py-2 text-small text-ink-2" aria-label="Lasting site changes">{flags.map(flag => <span key={flag.id}>↳ {flag.label}</span>)}</div>}
  </figure>;
}
