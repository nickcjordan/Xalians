...
export default function RouteComparison({ plans, selectedId, onSelect, onPreview, companion, recommendation }) {
  ...
  return <div className="lr-route-board" role="table" aria-label="Compare route costs and rewards" onPointerMove={previewColumn} onPointerLeave={restorePreview} onFocus={previewColumn} onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget)) onPreview?.(selectedId || null); }}>
    <div className="lr-board-row lr-board-head" role="row">
      <div className="lr-board-axis" role="columnheader">Choose your crossing</div>
      {plans.map((plan, index) => <div key={plan.route.id} role="columnheader" data-route-preview={plan.route.id} data-lowest-risk={plan.risk === lowestRisk ? 'true' : undefined} className={`${selectedId === plan.route.id ? 'is-selected' : ''}${recommendation?.plan.route.id === plan.route.id ? ' is-recommended' : ''}`}>
        <button type="button" className="lr-board-pick" aria-pressed={selectedId === plan.route.id} onClick={() => onSelect(plan.route.id)}>
    ...
    {rows.map(({ key, label, Icon }) => <div className={`lr-board-row is-${key}`} role="row" key={key}>
      ...
      {plans.map((plan, index) => {
        ...
        return <div key={plan.route.id} role="cell" data-route-preview={plan.route.id} className={`lr-board-value${selectedId === plan.route.id ? ' is-selected' : ''}`} aria-label={`${plan.route.title}: ${value} ${key}${unknown ? ' known, plus unknown extra cost' : ''}`}>
    ...
    {plans.some(plan => plan.method.abilityId) && <div className="lr-board-row lr-board-abilities" role="row">
      ...
      {plans.map(plan => <div role="cell" key={plan.route.id} data-route-preview={plan.route.id} className={selectedId === plan.route.id ? 'is-selected' : ''}>{plan.method.abilityId ? <><strong>Uses {plan.method.ability?.name || 'ability'}</strong><small>Unavailable afterward</small></> : <><Check aria-hidden="true" /><strong>All kept</strong></>}</div>)}
    </div>}
    {plans.some(plan => plan.route.consequence) && <div className="lr-board-row lr-board-future" role="row">
      ...
      {plans.map(plan => <div role="cell" key={plan.route.id} data-route-preview={plan.route.id} className={selectedId === plan.route.id ? 'is-selected' : ''}><strong>{plan.route.consequence?.label || 'No lasting change'}</strong><small>{plan.route.consequence?.future}</small></div>)}
    </div>}
    <div className="lr-board-row lr-board-footer" role="row">
      <div className="lr-board-axis" role="rowheader">Your route</div>
      {plans.map((plan, index) => <div role="cell" key={plan.route.id} data-route-preview={plan.route.id} className={selectedId === plan.route.id ? 'is-selected' : ''}>
