...
export default function FieldRecord({ scene, title, label, map, resources, children }) {
  ...
  return <div data-tier="immersive" data-field-record className="fixed inset-0 z-[1100] grid grid-rows-[auto_auto_minmax(0,1fr)] gap-2 overflow-hidden bg-room p-2 text-ink md:grid-cols-2 md:grid-rows-[auto_minmax(0,1fr)] md:gap-6 md:p-6" role="dialog" aria-modal="true" aria-label={label} onKeyDown={trapSequenceFocus}>
