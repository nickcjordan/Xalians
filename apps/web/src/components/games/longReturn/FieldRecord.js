...
export default function FieldRecord({ scene, title, label, map, resources, children }) {
  ...
  return <div data-tier="immersive" data-field-record className="fixed inset-0 z-[1100] grid grid-rows-[auto_auto_minmax(0,1fr)] gap-3 overflow-hidden bg-room p-3 pt-16 text-ink md:grid-cols-2 md:grid-rows-[auto_minmax(0,1fr)] md:gap-6 md:p-6 md:pt-20" role="dialog" aria-modal="true" aria-label={label} onKeyDown={trapSequenceFocus}>
    ...
    <div className="relative min-h-0 overflow-hidden md:col-start-2 md:row-start-2">
