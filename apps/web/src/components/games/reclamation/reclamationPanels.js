	...
	return (
		<Panel title="How to play" kind="help" onClose={onClose}>
			...
			<section className="rec-help-section">
				...
				<p>The rival is always above and in brass; you are always below and in cyan. Every number belongs to one side, in that side&apos;s color, and no number adds the two sides together.</p>
				<ul className="rec-help-marks">
					<li><b>The two bars across a world</b> are what each side would hold there after the Clash if the round ended now, the rival&apos;s above and yours below, on one scale for all three worlds, with each total at the end of its bar. The longer bar takes the world. A hatched end is what the Clash takes off that side. The rival&apos;s hidden creatures are not in them.</li>
					<li><b>The bar under a creature</b> is its hold, with its number. A hatched end is what the Clash would take from it; a cross means it would fall.</li>
					<li><b>The three columns on each card in your squad</b> are the three worlds, in the same order as above. A column&apos;s number and height are what your side there would gain by sending that creature there now: what it would still hold after the Clash, a lighter part for what it adds to your creatures already there, and hatched red on top what the Clash would take off it. A brass tag hanging from the top of a column is the rival&apos;s total at that world now and after that send (12&rarr;0: its 12 there would be driven to nothing). A brass pointer on a column&apos;s edge is what the rival would still lead by: a column that passes it would put you ahead, and is lit. Point at a world to light its column on every card.</li>
					<li><b>The dashed line across a card&apos;s columns</b> is what the creature holds at a world that neither helps nor hurts it. The marks under a column say why it stands above or below the line: a house on its home world (half again as much), a flame or a snowflake where the world is too hot or too cold, the world&apos;s air struck out where it cannot breathe (half, or a quarter).</li>
					<li><b>The top bar</b>: the round track (the game&apos;s nine worlds, three rounds of three, filled in the winner&apos;s color), then the score, a pennant for each world won, the rival&apos;s row above yours. First to {clinch} wins; a side one world from winning has its last pennant burning. The piece, the ticks and the number after each row are that side&apos;s sends left.</li>
