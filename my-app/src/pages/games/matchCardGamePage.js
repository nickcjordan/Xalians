...
class MatchCardGamePage extends React.Component {
	...
	handleCardClick = (i, event, xalianImageElementId, xImageElementId) => {
		if (this.state.started && !this.state.animating) {
			...
			gsap.timeline().fromTo('#match-game-start-button', { scale: 1 }, { scale: 2, duration: 0.5 }).fromTo('#match-game-start-button', { scale: 2 }, { scale: 1, ease: 'bounce.out' });
		}
	};
	...
	flipCardDown = (card) => {
		return gsap.timeline().to(`#${card.imageId}`, { scaleX: 0, duration: 0.1 }, '<').to(`#${card.xId}`, { scaleX: 1, duration: 0.1 });
	};
	...
	doIncorrectMatchAnimation = (xalianImageElementId, xImageElementId, selection) => {
		...
		let shake = gsap.timeline({ repeat: 1, yoyo: true }).to(`#${xalianImageElementId}, #${selection.imageId}`, { scale: 1.1, duration: 0.05 }).to(`#${xalianImageElementId}, #${selection.imageId}`, { rotate: -2, duration: 0.025 }).to(`#${xalianImageElementId}, #${selection.imageId}`, { rotate: 2, duration: 0.025 });

		let down = gsap.timeline().to(`#${xalianImageElementId}, #${selection.imageId}`, { scaleX: 0, duration: 0.1 }).to(`#${xImageElementId}, #${selection.xId}`, { scaleX: 1, duration: 0.1 });
	...
	doMatchAnimation = (xalianImageElementId, xImageElementId, selection, callBack) => {
		...
		let grow = gsap.timeline({ repeat: 1, yoyo: true }).to(`#${xalianImageElementId}, #${selection.imageId}`, { scale: 1.05, duration: 0.1 }).to(`#${xalianImageElementId}, #${selection.imageId}`, { opacity: 1, duration: 0.05 }).to(`#${xalianImageElementId}, #${selection.imageId}`, { opacity: 0.2, duration: 0.05 });
		let dim = gsap
			.timeline()
