...
`encyclopedia.json` is the public structured file. The flip happened on 2026-09-02: `scripts/bundleLore.js` copies it, `chronicle.json`, `docs/species-templates/registries.json`, and the species records listed in `docs/species-templates/RATIFIED.json` (as `speciesRecords.json`) into `lambda/src/json/`, and `copy-json` mirrors them into the frontend, where `my-app/src/lore/` is the only reader and the `/encyclopedia` page renders them (contract: `docs/design/xalian-encyclopedia-page.md`). Species entries from the records are merged into the index at load time under `xalians`; they are not written into this file. The old `/glossary`, `/planets`, and `/species` pages remain until Nick retires them. This companion file stays out of the bundle permanently.
