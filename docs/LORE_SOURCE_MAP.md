...
One clean map of where canon lives, what each source contributes, and what is explicitly NOT canon. Compiled 2026-08-30 from the repo's data files, the lore-voice skill references, and the full Evernote export (`notes/Cosmo Labs.html` plus its two attachments). When sources disagree, this document says which one wins.
