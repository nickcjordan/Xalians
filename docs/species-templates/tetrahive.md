...
- Small bat-shaped flier
- Light
- Broad membranous wings
- Upright ears
- Hooked claws on the forelimbs and feet
- Long thin tail
- Surrounded by a swarm of summoned flying familiars, toothed like piranhas
...
- **feeding**: It eats what its swarm brings down.
...
- **company**: It goes with its swarm and nothing else.
...
Fields required (Nick, 2026-09-10): filled feeding ("It eats what its swarm brings down."); company ("It goes with its swarm and nothing else."). Source reading: the teaser, the diet key and the planet history; where they are silent the phrase is the plainest reading of the record and is flagged for Nick with the batch.
...
`foresighted` now sits at 100 on this species, which is the first time a trait the registry calls rare and bands at 2 to 8 percent has been written as universal. The iteration-three ruling is explicit that an unqualified fauna observation naming a trait puts it at 100, and Grimedes' report states the anticipatory-response finding without any minority qualifier, so I applied it as written rather than working around it. The friction is real: if every Grimedes native carries `foresighted` at 100, the trait stops distinguishing individuals on that planet and the rarity band in the registry no longer describes anything. The smallest fix I can see is to let the report's fauna observations name a trait at a high but sub-universal percent (say 85) when the registry marks that trait rare, which keeps the demand visible without spending the rarity band. Nick decides; the entry stays at 100 until he does.
