...
Expected count: the percents sum to 575, so the expected number of traits per individual is 5.75. There is no exclusion pair active: neither `pack-bonded` nor `solitary` is listed, so no exclusion adjustment applies. `protective` at 70 and `foresighted` at 5 keep two entries strictly between 0 and 100, and no entry at 100 has an exclusion partner. Revised 2026-09-07 by the trait evidence bar, again the same day by iteration two, and on 2026-09-08 by iteration three of that bar: the cut table below lists what is still removed, and the surviving pool stands at 5.75 expected traits.
...
- `luminous` 100. Raised from 60 on 2026-09-08. The species sentence states the glow of the whole species and states it as what a Hypnopet is, species: "Hypnopets resemble glowing, golden-furred bunny rabbits with a single color-changing unicorn horn atop their heads." That is the plainly universal case, so the percent sits at 100. The iteration two argument for holding it lower rested on the horn beginning to pulse, but the horn display and the golden coat glow are two different things and only the display is described as starting; the coat is simply how the animal is drawn and described. This is a line call worth review.
- `perceptive` 100. Raised from 45 on 2026-09-08. `physiology.senses.special` carries `psychic`, and iteration three rules that every special sense names the trait as a species-wide fact, which places it at 100. The graded bands support the same reading independently, since `physiology.senses.hearing` is banded 70 to 90 and clears the tightened bar of an upper bound at or past 80.
- `regenerative` 100. New on 2026-09-08. Nick's legacy ratings in `lambda/src/json/species.json` set `recoveryRating` to high for this species, and iteration three maps a high recovery rating onto this trait as a species-wide fact at 100. The iteration one and two walkthroughs left it out on the argument that the healing is done to others and never to itself; the rating is the ratified statement that this body also comes back fast, and a demanded trait is not hidden behind a reading of the prose.
- `foresighted` 5. Restored on 2026-09-08 as an ordinary rolled entry, not a demanded one. The Generator Environmental Report for Telypso records anticipatory-response forms in its fauna observations, which is evidence that the world's Generator produces them, but the line enumerates form types within the population rather than stating a fact of the whole population, and the Hypnopet is plainly one of the trance-induction and empathic forms in the same list. So the report opens the door to a rare roll rather than naming the trait species-wide. Held at the percent it carried before the iteration one cut, inside the registry's rare band for this key. This is the second line call in this pool: an orchestrator reading the same line as universal would put it at 100.
...
- 2026-09-02, conduits (Nick): `conduits` {"crest":"psychic"}. crest conducts psychic: the horn pulses and swirls with color to hypnotize.
