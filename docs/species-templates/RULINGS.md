...
- 2026-09-10 (Nick, later the same day): the five short fields are required on every species. Games read the structured physiology and never parse these fields; the fields are presentation text (encyclopedia, dossiers, card backs), and a presentation consumer breaks visibly on a missing field, so the blank is filled once in the record rather than by every app with its own default. Where the canon is silent the field carries the plainest reading the record supports; a lost purpose is written in the encyclopedia voice ("No record says what it was made for"), never invented. Company is never defaulted to solitary silently; each authored company line is flagged for Nick with the batch. No structured company key is added to physiology until a game asks for one. Filled on this ruling: 3 origins (Thirstaserp, Voltish, Xylum), 15 feeding lines from the diet key, 25 company lines.
- 2026-09-10 (Nick): habits split. The habits paragraph and biomeNiche are struck for five short optional fields under lore: origin, habitat, feeding, behavior, company. Short phrases allowed; never pad a phrase into a sentence, never cut words for brevity; omit a field with no source; planet history stays in the planet records; no sign-off lines; no hazard sentences written to justify traits; the template phrase "its ground" is struck. All 30 records converted; fields are draft until Nick signs off. Why: the paragraph form was a checklist forced into prose.
