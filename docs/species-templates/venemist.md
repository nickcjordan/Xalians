...
- `resistant: 100`: environment-demanded, and raised to 100 by iteration three. Three separate sources name it as a species-wide fact: the Drainov Generator Environmental Report records full-spectrum toxin immunity as universal in the planet's fauna, lists chemical immunity and filtration organs among its output priorities, and names the atmosphere first in its hazards, which is chemically hostile air. A demanded trait sits at 100, so the former 95 no longer stands. Original iteration-one reasoning follows, unchanged in substance, and the registry allows a planet-wide sentence to justify an environmental adaptation. Planet: "The Xalians on Drainov quickly adapted to not only the smog and industrial waste common to the world but also to its bubbling acid swamps, steaming corrosive rain, asphyxiating fumes, slow-drifting clouds of neurotoxic gases, and virulent pathogens." Left at 95 rather than 100 so the pool carries variance and because the species sentence itself does not name resistance. Source `planet`.
...
- Expected trait count after iteration three: (100 + 100 + 100 + 35) / 100 = 3.35. `perceptive` at 35 is the entry below 100 the model requires; no exclusion pair remains, so no correction applies.
- `perceptive` re-checked under iteration three ruling 1 and kept at 35: it rests on graded sense bands rather than a special sense, and `physiology.senses.smell` tops out at 85, above the new bound of 80. The hearing band tops out at 80, which also clears it. Nothing raises it: no special sense is listed and no output priority on Drainov names non-visual sensory systems.
- `resistant` re-checked under iteration three ruling 3 and kept: its support is the toxin, acid, corrosive-rain and pathogen load of Drainov, which is contamination and chemically hostile air, the exact scope the narrowed definition retains. No part of it rests on thin atmosphere, dust, static discharge or temperature.
