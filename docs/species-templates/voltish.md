...
- `perceptive: 100` : raised to 100 by iteration three, since `physiology.senses.special` carries `electroreception` and a special sense names this trait as a species-wide fact. The iteration-two reasoning, at the former 35, follows: restored 2026-09-07 under class 4, a ratified field of the record. `physiology.senses.special` carries `electroreception`, which the widened bar names outright as justification for this trait. Held at the authored 35, since a special sense is a body fact rather than a sentence about finding what hides.
- `armored: 10` : restored 2026-09-07 under class 4, a ratified field of the record. `physiology.composition.secondary` is `metal`, and the widened bar reads a metal composition as justification for armored. Kept in single figures, as authored, because the metal is skeleton and claw while the covering at rest is fur.
- `insulated: 100` : demanded species-wide. The Zolton Generator Environmental Report lists insulation first among its output priorities, names continuous lightning first among its hazards, and records insulated climbing forms in its fauna observations. Charge passing over or through the body without harm is the only way a native of a world struck 2.5 billion times a local day survives it, and this species feeds on that charge. New entry.
- `hardened: 100` : demanded species-wide. Cryogenic canyon exposure is a hazard entry for Zolton and the terrain features name canyon systems saturated with dense cryogenic gas, which is deep cold, a physical extreme rather than a contaminant. The record already carries the tolerance as a temperature floor of minus 55, and the trait is what that floor rests on. New entry, and the fact that used to be argued under `resistant` before the 2026-09-08 narrowing.
- `pack-bonded: 25` : restored at its authored percent as ordinary evidence, not as a demanded trait. The Zolton Generator Environmental Report records that entangled pair lineages persist from the discontinued relay program and that paired individuals register synchronized responses at arbitrary separation. That line is qualified to specific lineages rather than the population, so it is rolled evidence rather than a species-wide fact. `solitary` stays cut, so no exclusion correction applies.
...
Expected trait count after iteration three: (100 + 100 + 100 + 100 + 75 + 45 + 25 + 10) / 100 = 5.55. Four entries sit below 100 (`volatile` 75, `menacing` 45, `pack-bonded` 25, `armored` 10), which satisfies the model's requirement, and `solitary` is still cut, so the exclusion pair has no correction. This is a heavy pool, well above the 3.5 guideline, and it is reported to the orchestrator rather than trimmed: every one of the four entries at 100 is demanded by a named source, and the rule forbids hiding a demanded trait behind a percent.

`resistant` re-checked under iteration three ruling 3 and kept at the raised 100: its support is the neutron radiation of Zolton's black lightning, which the narrowed definition still names directly. The cryogenic canyon exposure in the same hazard list is now `hardened` and the continuous lightning is now `insulated`, so neither of those facts leans on `resistant` any longer.

`armored` re-checked and left at 10: the demanded-trait rule names chitin, plating, crystal, shell or metal armor, and this body's metal is a skeleton and a set of claws under a fur covering, not armor, so nothing raises it. (The pool total before this pass was 3.45 by the arithmetic of the percents; the 3.15 figure in the heading above was a miscount carried in the walkthrough.)
