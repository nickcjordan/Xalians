...
- `perceptive: 35` : restored 2026-09-07 under class 4, a ratified field of the record. `physiology.senses.special` carries `electroreception`, which the widened bar names outright as justification for this trait. Held at the authored 35, since a special sense is a body fact rather than a sentence about finding what hides.
...
| `pack-bonded` | 25 | class 3 fails: the entangled-pair line in the Generator report is a planet fact, and a planet fact may support an environmental adaptation only, never a coordination behavior |
...
Expected trait count after iteration two: (90 + 75 + 45 + 35 + 10) / 100 = 2.55. All five surviving entries sit below 100, which satisfies the model's requirement, and the exclusion pair is gone, so no correction applies.

`resistant` re-checked under ruling B and kept: its support is the neutron radiation of Zolton's black lightning, which the trait definition names directly. The cryogenic canyon exposure in the same hazard list would not have carried it alone. (The pool total before this pass was 3.45 by the arithmetic of the percents; the 3.15 figure in the heading above was a miscount carried in the walkthrough.)
