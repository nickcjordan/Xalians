...
**Trait pool.** Pool shape, 2026-09-08 ruling: one required trait and five rolled entries whose shares sum to 100. Iteration three had four entries at 100; one of them is what this species is in its own lore and the rest drop into the rolled set, and two entries leave the pool entirely because it holds at most six.

**Required**

| Trait | Evidence |
|---|---|
| `insulated` | The planet-wide adaptation that this species' own description makes the point of the creature, which is the case the ruling admits as required. `species`: "can store electrical energy from its surroundings and release the shock into enemies", on a world whose canyons carry visible rivers of electricity over the crust, and the record's `diet` is `energy-feeder`, so current entering this body without harming it is the precondition for everything the creature does. The Zolton Generator Environmental Report agrees from the other side: insulation is first among its output priorities, continuous lightning is first among its hazards, and insulated forms appear in its fauna observations. |

**Rolled (shares sum to 100)**

| Trait | Share | Evidence |
|---|---|---|
| `volatile` | 30 | Strongest of the rolled set, and the only rolled entry carried by a sentence about the species' own body. `species`: "can store electrical energy from its surroundings and release the shock into enemies". A body holding stored charge is hazardous to strike. It is rolled rather than required because the registry defines the trait as reacting when hit, while the description shows deliberate release, so the fit is partial (see Friction below). |
| `menacing` | 22 | A species sentence, `species`: "this daunting creature". The word is applied to this species and to nothing else, which puts it in the strongest evidence class, but it is an adjective on appearance rather than a shown effect on courage. |
| `resistant` | 20 | A planet hazard covering this form. Black lightning neutron emission is a hazard entry in the Zolton Generator Environmental Report, and radiation sits inside the narrowed scope of the trait. `planet`: "black lightning was so intense that it generated low-yield nuclear fusion, generating a fatal neutron burst that released lethal neutron radiation into the atmosphere, effectively randomly killing off all organic life within several kilometers of the strike zone". Demoted from 100: the species' own description is about charge, not about radiation, so the world demands it of everything alive there rather than of this creature in particular. |
| `hardened` | 16 | A second planet hazard, ranked below the first. Cryogenic canyon exposure is a hazard entry for Zolton and the terrain features name canyon systems saturated with dense cryogenic gas, which is deep cold, a physical extreme rather than a contaminant. The record already carries the tolerance as a temperature floor of minus 55. Demoted from 100. |
| `perceptive` | 12 | A record field, the weakest class kept. `physiology.senses.special` carries `electroreception`, which the senses list makes evidence for the trait but never enough to make it required, since a required trait must not restate a field the record already carries. Demoted from 100. |

#### Cut (2026-09-07 evidence bar, and 2026-09-08 pool shape)
...
Expected trait count after the pool shape: (100 + 30 + 22 + 20 + 16 + 12) / 100 = 2.00, down from the 5.55 of iteration three. The chance an individual lands none of the five rolled traits is 0.70 times 0.78 times 0.80 times 0.84 times 0.88, which is 32.3 percent. Pool size is six entries, exactly at the cap, which is why `pack-bonded` and `armored` are cut above rather than kept at a small share.

`resistant` re-checked under the narrowed 2026-09-08 definition and kept in the pool: its support is the neutron radiation of Zolton's black lightning, which the definition still names directly. The cryogenic canyon exposure in the same hazard list is `hardened` and the continuous lightning is `insulated`, so neither of those facts leans on `resistant`. All three are planet-wide, and only the one the species' own description is built on is required.

`armored` is now out of the pool entirely rather than sitting at 10: the demanded-trait rule names chitin, plating, crystal, shell or metal armor, and this body's metal is a skeleton and a set of claws under a fur covering, so it was the weakest of eight candidates for six slots.
...
Zolton report line, resolved 2026-09-08: the Generator Environmental Report for Zolton carried a universal-adaptation line reading energy conduction and redirection rather than storage, which contradicted this species outright, since the whole of the Voltish description is that it can store electrical energy from its surroundings and release the shock into enemies. The record could not both follow its own source sentence and agree with the planet. Nick softened the report line on 2026-09-08 to allow storage in a minority of lineages, so conduction and redirection remain the world's common answer to continuous lightning while the Voltish is the admitted exception the report itself makes room for. Record and planet now agree, and nothing in this template changes: `insulated` stays required, `volatile` stays the top rolled entry, and the `energy-feeder` diet keeps its basis.
...
Final run, `node docs/species-templates/tools/validate-template.js voltish`:

```
WARN lore.description.lineage       lore.description contains gendered or lineage language (Xalians have no sex or lineage); confirm it is not about the species
...
Both WARNs are answered in the Validator warnings answered section above.
