...
Expected trait count: the six surviving percents sum to 480, so an individual carries about 4.80 traits. No exclusion pair is listed, so no adjustment applies, and `anchored` at 55 and `menacing` at 25 keep two entries below 100. Revised 2026-09-07 by the trait evidence bar, again the same day by iteration two, and on 2026-09-08 by iteration three of that bar: the cut table below lists what is still removed, and the surviving pool stands at 4.80 expected traits.
...
- `resistant: 100`: raised from 40 on 2026-09-08. The hazard list in the Generator Environmental Report for Floria carries toxic fungal blooms, and a hazard of the home planet names the adaptation every native has to carry, which puts the trait at 100. The planet history says the same thing in prose, planet: "massive infestations of toxic fungi." A body that works the underforests full time is exposed to it every day of its life, and this is contamination rather than temperature, so the narrowing of 2026-09-08 leaves the key unchanged.
...
- `regenerative: 100`: raised from 25 on 2026-09-08. The Generator Environmental Report for Floria lists regenerative tissue among the world's three output priorities, and an output priority that names a trait names it as a species-wide fact of that world's stock. The iteration two reading is unchanged, only its weight: this is the Generator's own stated output for every body it prints on Floria, so the percent is no longer held down by the absence of a sentence showing this body closing a wound.
- `territorial: 100`: new on 2026-09-08 under the key added the same day. The fauna observations for Floria record a territorial strategy of growth, entanglement, and regrowth in place of engagement, stated of the population with no quantifier at all, which the iteration three rule reads as naming the trait species-wide. The registry key is denial of passage rather than pursuit, which is exactly the strategy described, and a browsing body with a blade as long as itself standing in the brush it clears is the shape of it. This is a line call: the observation describes the world's plant-side habit of regrowing across an intruder, and a reader could hold that a mobile quadruped only partly inherits it.
- `perceptive: 100`: new on 2026-09-08. `physiology.senses.special` carries `tremorsense`, and iteration three rules that every special sense names the trait as a species-wide fact, which places it at 100. The graded bands would not have carried it on their own, since sight, hearing and smell all top out at 70 or below, under the tightened bar of 80.
