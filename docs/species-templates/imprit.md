...
Re-run 2026-09-10 against the ratified `lore.appearance` list and five short fields, now source text on the same footing as the teaser, the art and the planet record. One required trait at 100, five rolled entries whose percents sum to exactly 100, and six entries in all, which is the ceiling. Expected count is the sum of all percents divided by 100, (100 + 100) / 100 = 2.00, unchanged from before the re-run. An individual lands no rolled trait at all (1-.30)(1-.30)(1-.20)(1-.15)(1-.05) = 32.1 percent of the time. `solitary` is the only exclusion-eligible entry in the pool and `pack-bonded` is not listed, so no exclusion adjustment applies to the expected count.

Required

| Trait | Evidence |
|---|---|
| `hardened` | The body fact the species is built on, and the planet-wide adaptation the species' own description makes the point of the creature, which is the case the pool shape keeps at 100. Evidence (species): "Imprits possess fire-retardant fur that protects their bodies from the flammable oils they secrete, allowing them to survive despite being in a state of constant immolation." Confirmed by the ratified appearance entries "Fire-retardant fur" and "Wreathed in constant flame". The Magmuth report says the same thing three ways, with thermal shielding first among its output priorities, heat-shielded integument recorded as universal in its fauna observations, and firestorms, pyroclastic flows and ambient temperature excursions in its hazard list. A creature that lives inside its own fire is the case this key exists for. |

Rolled (shares sum to 100)

| Trait | Share | Evidence |
|---|---|---|
| `solitary` | 30 | Raised from the rolled floor. The ratified `company` field states it in the present tense and without qualification: "It keeps to itself, shaped by the isolation of the deep tunnels." That is a direct, current-habit sentence, a stronger evidence class than the history-only reading the trait held before the fields existed. `pack-bonded` is deliberately absent, so at most one of the pair can land, as the shape requires. |
| `luminous` | 30 | A species sentence about the species' own body: a body in permanent flame sheds light. Evidence (species): "in a state of constant immolation". Reinforced by the ratified appearance entry "Wreathed in constant flame," a second, independent statement of the same body fact. Still demoted from 100 because the covering field is `fur` and no light organ is recorded, so the glow is read from the description and appearance rather than stated by the body fields, and the required slot is already spent on the same underlying fact. |
| `slippery` | 20 | Nick's legacy `statRatings` set `evasionRating` to high for this species, and the species sentence reads the same way, a body that lives by swinging out of reach. Evidence (species): "using their tails to swing through the air from place to place". Share reduced to make room for `solitary`'s stronger, newly ratified evidence; the trait itself is unweakened. |
| `volatile` | 15 | A species sentence about the species' own body again: anything striking this animal strikes burning fur soaked in flammable oil. Evidence (species): "the flammable oils they secrete". Ranked below `luminous` because the sentence states the secretion rather than a reaction to being hit. |
| `nocturnal` | 5 | A species sentence about where the animal was made to work. Evidence (species): "let loose into the various lightless mining shafts of Magmuth". Weakened further by the ratified `behavior` field, "On the surface it is erratic, and it sets fires where they least belong," which puts the present-day animal above ground and acting by day as readily as by night; the shafts are its origin, not a standing night-adaptation. Held at the rolled floor rather than cut, since the ratified `habitat` field still names "the old mining shafts of Magmuth" as part of where it lives now. |

Traits I considered and left out, so their chance is zero: `armored`, since there is no armored aspect anywhere and the covering is fur with the anatomy key `hide`; `regenerative`, no source; `anchored`, the opposite of everything the description shows; `ramming`; `protective`, `healing` and `inspiring`, since nothing shows it acting for others; `mind-sealed`, since the description says its mind broke, which is the reverse; `hypnotic`; `foresighted`; `telekinetic`; `phasing`; `reflective`; `pack-bonded`, excluded in favor of `solitary`, now more strongly so on the ratified company field; and `stealthy`, because a body in permanent visible flame cannot move unseen and `luminous` is the honest opposite entry. The ratified `origin` and `feeding` fields ("Designed by Magmuth's corporations as a tinkerer..."; "It eats what it finds in the shafts and on the surface, whatever it is.") restate facts already carried elsewhere in the record and name no further trait.

#### Trait re-run (2026-09-10)

- `solitary` raised from 10 to 30, the largest single change. The 2026-09-07/08 pool held it at the rolled floor because the only evidence was, in the walkthrough's own words, `a species sentence about its history rather than its present habit`. The ratified `company` field replaces that with a plain, present-tense statement, "It keeps to itself, shaped by the isolation of the deep tunnels," which is exactly the missing ratified-field evidence.
- `luminous` raised from 25 to 30. Unchanged source fact (constant immolation), but now independently confirmed by the ratified appearance entry "Wreathed in constant flame," which is a second, separate statement of the same body fact rather than an inference from the teaser alone.
- `slippery` lowered from 30 to 20, `volatile` lowered from 20 to 15, and `nocturnal` lowered from 15 to 5, to keep the rolled sum at exactly 100 once `solitary` and `luminous` grew. None of the three lost evidence; `nocturnal` additionally weakens on the ratified `behavior` field's "On the surface it is erratic," which shows present-day activity is not confined to night or to the shafts.
- No trait was added to or dropped from the six-entry set: the pool was already at the ceiling, so the re-run reallocates the existing 100 rolled points rather than adding a seventh entry.
- `pack-bonded` stays out: the new `company` field settles the exclusion pair toward `solitary`, not away from it.

#### Cut by the evidence bar (2026-09-07)

| Trait | Former percent | Evidence class that failed |
|---|---|---|
| `toxic` | 12 | plausibility from the flammable oil; no sentence shows it harming by chemistry. Not restored: no ratified field or appearance entry states a chemical or toxic effect. |
| `resistant` | 100 | rule three of iteration three: under the narrowing of 2026-09-08 the key covers contamination only, and every fact behind this entry was thermal. Magmuth's hazard list names no toxin, no corrosive and no pathogen, its ash is particulate rather than chemical, and the fire-retardant fur answers burning. The fact itself is not lost: it moves to `hardened` at 100. Not revisited: the ratified fields add no contamination fact. |
| `menacing` | 25 | pool full, weaker evidence. Not restored: no ratified field states a threat display, and the pool is still full at six entries. |
| `perceptive` | 10 | pool full, weaker evidence. Not restored: no ratified field strengthens a sense claim, and the pool is still full at six entries. |

Pool expected count 5.15 to 4.93 in iteration one, then 4.93 to 5.03 in iteration two once `perceptive` was restored. Iteration three on 2026-09-08 moved it from 5.03 to 5.31: `resistant` cut and replaced by `hardened` at 100, `slippery` raised to 100 on the legacy evasion rating. `perceptive` at 10 was re-checked under the tightened sense rule and survives, because `physiology.senses.hearing` is banded 60 to 80 and an upper bound of 80 meets the bar exactly. The iteration two defense of `resistant` no longer holds: the secreted oil is the body's own product and what the fur answers is the burning, so the honest key is the physical one. No surviving rolled percent moved. The pool shape of the same date then replaced iteration three: `hardened` is the single required trait, `slippery` and `luminous` fell out of the demanded set into the rolled one, and the five rolled shares were authored to sum to exactly 100. `menacing` and `perceptive` were cut for the six-entry ceiling; both keep the evidence they had, an appearance sentence and a hearing band topping out at 80, but they are the weakest candidates and the rolled slots went to stronger ones. Expected count 5.31 to 2.00. The 2026-09-10 re-run reallocates within that same 2.00 expected count; see the subsection above for what moved and why.
...
1. `traits.expected`: the expected trait count is 4.55, above the script's 3.5 comfort line. I confirm the species is meant to carry that many, and the reason is specific to this body rather than generous authoring. Two of the nine entries are near-certainties forced by one sentence, the sentence about fire-retardant fur over secreted flammable oils on a permanently burning body: that single fact demands `hardened` at 100 (`resistant` until the narrowing of 2026-09-08) and very nearly demands `volatile`, and it also produces `luminous`. A creature whose defining feature is that it is always on fire simply carries more passive facts than a creature whose defining feature is that it is large. Removing an entry would mean deleting a sourced fact to hit a number, so I left the pool as authored and am recording the friction here instead.
...
Final run after the trait re-run of 2026-09-10 (superseding the run above, which predates the pool-shape pass of 2026-09-08 and the lore split of 2026-09-09: `conduits.tail` and the `traits.expected` warning it quotes no longer apply, since `conduits.tail` was removed by the 2026-09-02 orchestrator amendment and the pool now sits at expected count 2.00):
...
`temperature.planet` is answered under Dromeus's Validator output note (same standing cause, unrelated to the trait pool). `instruments.predicate.source` and `conduits.source` for `secretion` are answered in the Script warnings section above (numbers 2 and 4): the emitted-substance predicate and the fire conduit both rest on the quoted sentence, "Imprits possess fire-retardant fur that protects their bodies from the flammable oils they secrete, allowing them to survive despite being in a state of constant immolation." The trait checks `traits.pool.required`, `traits.pool.rolledSum` and `traits.pool.size` are all silent; no `traits.expected` warning is raised, since the re-run kept the expected count at 2.00.
...
- 2026-09-10, trait re-run: pool re-derived against the newly ratified `lore.appearance` list and five short fields. `solitary` raised 10 to 30 on the `company` field, "It keeps to itself, shaped by the isolation of the deep tunnels."; `luminous` raised 25 to 30, reinforced by the appearance entry "Wreathed in constant flame"; `slippery` 30 to 20, `volatile` 20 to 15, `nocturnal` 15 to 5 to keep the rolled sum at 100; expected count stays 2.00.
