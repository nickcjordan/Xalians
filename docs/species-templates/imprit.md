...
Reshaped on 2026-09-08 under Nick's pool shape: one required trait at 100, five rolled entries whose percents sum to exactly 100, and six entries in all, which is the ceiling. The expected count falls from 5.31 to 2.00, and an individual lands no rolled trait at all 32.1 percent of the time. `solitary` is the only exclusion-eligible entry and `pack-bonded` is not listed, so no exclusion adjustment applies.

Required

| Trait | Evidence |
|---|---|
| `hardened` | The body fact the species is built on, and the planet-wide adaptation the species' own description makes the point of the creature, which is the case the pool shape keeps at 100. Evidence (species): "Imprits possess fire-retardant fur that protects their bodies from the flammable oils they secrete, allowing them to survive despite being in a state of constant immolation." The Magmuth report says the same thing three ways, with thermal shielding first among its output priorities, heat-shielded integument recorded as universal in its fauna observations, and firestorms, pyroclastic flows and ambient temperature excursions in its hazard list. A creature that lives inside its own fire is the case this key exists for. |

Rolled (shares sum to 100)

| Trait | Share | Evidence |
|---|---|---|
| `slippery` | 30 | First of the rolled set by rule: Nick's legacy `statRatings` in `lambda/src/json/species.json` set `evasionRating` to high for this species, which ranks the trait first among the rolled entries without setting an absolute percent. The species sentence reads the same way, a body that lives by swinging out of reach. Evidence (species): "using their tails to swing through the air from place to place". |
| `luminous` | 25 | A species sentence about the species' own body: a body in permanent flame sheds light. Evidence (species): "in a state of constant immolation". Demoted from 100 because the covering field is `fur` and no light organ is recorded, so the glow is read from the description rather than stated by the body fields, and the required slot is already spent on the same sentence. |
| `volatile` | 20 | A species sentence about the species' own body again: anything striking this animal strikes burning fur soaked in flammable oil. Evidence (species): "the flammable oils they secrete". Ranked below `luminous` because the sentence states the secretion rather than a reaction to being hit. |
| `nocturnal` | 15 | A species sentence about where the animal was made to work. Evidence (species): "let loose into the various lightless mining shafts of Magmuth". Held low because Magmuth is not a world of perpetual night and the present-day description has the animal out of the shafts and on the surface. |
| `solitary` | 10 | Weakest of the set: a species sentence about its history rather than its present habit, the isolation of the deep tunnels being what the description says shaped it. `pack-bonded` is deliberately absent, so at most one of the pair can land, as the shape requires. |
