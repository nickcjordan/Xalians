...
Eight entries, expected count 5.31 traits per individual before tilts and before the exclusion rule. There is only one exclusion-eligible entry here, `solitary`, with no partner listed, so the expected count needs no adjustment. Revised 2026-09-07 by the trait evidence bar, again the same day by iteration two, and on 2026-09-08 by iteration three of that bar: the cut table below lists what is still removed, and the surviving pool stands at 5.31 expected traits.

- `hardened: 100`, environment and body demanded, new on 2026-09-08. The Generator Environmental Report for Magmuth names it three ways: the output priorities open with thermal shielding, the fauna observations record heat-shielded integument as universal with no qualifier at all, and the hazard list carries firestorms, pyroclastic flows and ambient temperature excursions. Every one of those is thermal load, which the key added on 2026-09-08 now owns. The species sentence says the same thing about this body in particular. Evidence (species): "Imprits possess fire-retardant fur that protects their bodies from the flammable oils they secrete, allowing them to survive despite being in a state of constant immolation." A demanded trait sits at 100, and this one is demanded by the world and by the body at once.
...
- `slippery: 100`. Raised from 72 on 2026-09-08. Nick's legacy ratings in `lambda/src/json/species.json` set `evasionRating` to high for this species, and iteration three rules that a legacy rating naming a trait puts it at 100 rather than at an authored percent. The reading is unchanged: a swinging body that lives by not being held. Evidence (species): "using their tails to swing through the air from place to place".
...
- `luminous: 100`, body demanded. A body in permanent flame sheds light, and the description states the flame of every individual. Evidence (species): "in a state of constant immolation". The percent in the template has been 100 since the pool was written and the expected count has always been computed from 100; the 40 that stood in this line until 2026-09-08 was a stale number in the prose, corrected here rather than carried.
