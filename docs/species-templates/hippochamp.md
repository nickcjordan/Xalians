...
**Trait pool.** Sum of percents is 445, so the expected trait count is about 4.45. There is no exclusion pair in the pool, so no adjustment applies. `protective` at 85 and `pack-bonded` at 60 keep two entries below 100, so every individual differs. Revised 2026-09-07 by the trait evidence bar, again the same day by iteration two, and on 2026-09-08 by iteration three of that bar: the cut table below lists what is still removed, and the surviving pool stands at 4.45 expected traits.
...
- `resistant` 100. Raised from 70 on 2026-09-08 because the trait is demanded species-wide by the Generator Environmental Report for Poseidas twice over: the hazard list carries semiannual toxic bloom events with airborne microbial dispersal at the surface, and the output priorities carry filtration metabolism. Both are contamination facts every native body has to answer, and a demanded trait sits at 100. The species sentence about the electrical and chemical fires on the rigs supports the same reading and no longer has to carry it alone.
- `hardened` 100. New on 2026-09-08 under the key added the same day. The Poseidas hazard list carries crush depth and the output priorities carry depth tolerance, both of which name the physical-extremes key rather than the contamination one under the narrowing of the same date. The species sentence puts the same body inside burning rig fires as its working day, which is thermal load on top of pressure. Demanded environmentally, so 100.
- `territorial` 100. New on 2026-09-08, from a species sentence that states the fact for the whole species: species, "Four-legged creatures resembling a seahorse that patrol the various hydro-processing fleets on Poseidas, Hippochamps were designed as emergency response crews for the various electrical and chemical fires that occur on rigs, as well as a means of defense against pirates seeking to steal valuable Algael." Patrolling assigned ground and standing between raiders and the Algael is denial of passage, which is the registry definition, and it is the engineered purpose of every individual rather than a disposition some carry. That is the plainly universal case the rule places at 100, and it is the one line call in this pool worth a second look.
