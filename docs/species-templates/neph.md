...
- Nearly weightless for its size
...
| Trait | Evidence |
|---|---|
| `volatile` | The body fact the species' own description states, and the one that decides what this animal is on contact. Evidence (species): "being composed almost entirely hydrogen means that the Neph can also spray their enemies with bouts of flammable gas", with the planet history recording the same composition when it says entire flocks of Neph, being 95 percent hydrogen, were being ignited. Every Neph is hazardous to strike, so the fact is universal and takes the required slot. |
