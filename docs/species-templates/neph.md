...
Expected count is 3.90 traits per individual (`volatile` 1.00 + `slippery` 1.00 + `resistant` 1.00 + `pack-bonded` 0.90). There is no exclusion pair in this pool because `solitary` is not listed, so no exclusion adjustment applies, and `pack-bonded` at 90 is the entry that keeps individuals different. Revised 2026-09-07 by the trait evidence bar, re-checked the same day under iteration two with no change, and revised again on 2026-09-08 by iteration three: the cut table below lists what is still removed, and the surviving pool stands at 3.90 expected traits.
...
- `slippery` 100: raised from 70 on 2026-09-08. The fauna observations in the Generator Environmental Report for Saiphus record minimal restraint tolerance across all lineages, a statement carrying a universal quantifier and therefore naming the trait as a species-wide fact of every native, which sits at 100. It reads directly onto a herded species that people try to hold: the registry key is a body that cannot be pinned or snared, and a pastoral stock with minimal restraint tolerance is exactly that. The species-specific sentence that carried the entry before says the same thing, planet, "Their amorphous forms allowed them to be swept up and deposited harmlessly by the hurricane-force winds". This is the line call in this pool, because the observation could also be read as a note on temperament under management rather than on the body.
- `resistant` 100: raised from 45 on 2026-09-08. The Saiphus hazard list carries sulfuric cloud contact and the output priorities carry atmospheric filtration, and either one names the adaptation every native must carry, which places the trait at 100. The planet history states the same fact, planet, "Sulfuric acid clouds sweep haphazardly across the sky, choking the life out of anything they happen to touch". The iteration two reason for holding it low, that the clouds kill what they touch, measures the hazard rather than the answer to it, and a demanded trait is not written down for that. This is corrosion and hostile air, so the narrowing of 2026-09-08 leaves the key unchanged; a body that grazes the deep atmosphere all day is the case the key was written for.
- Left out and why: `armored` (no armored covering and no `shell`); `anchored` (a body that the wind carries is the opposite); `regenerative` (no source); `toxic` (its discharges are cold and flammable, not debilitating agents); `reflective`, `hypnotic`, `foresighted`, `telekinetic`, `mind-sealed`, `stealthy`, `healing` (nothing in either source); `nocturnal` (Saiphus is not a night world; the history describes sunrises across the whole sky); `luminous` (the planet history's bioluminescent zooplankton are a different organism, and reading that onto the Neph would be exactly the planet-wide-to-species error the rules forbid); `inspiring`, `ramming`, `phasing`, `solitary`; and `perceptive`, which was re-checked on 2026-09-08 and stays at 0, since `physiology.senses` carries no `special` entry and no source names a sense of any kind, the smell band of 55 to 80 being authored from the grazing habit rather than stated.
