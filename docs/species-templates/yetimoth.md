...
Pool shape, 2026-09-08 ruling: two required traits and four rolled entries whose shares sum to 100. Iteration three had three entries at 100 and an expected count of 5.30; this is the one species in the batch that keeps two required traits, and the reasoning is set out below.

**Required**

| Trait | Evidence |
|---|---|
| `armored` | The act the description and the signature ability are both built around, and the case the widened definition names. Under the 2026-09-08 registry the key covers natural plating, shell, an integrated chassis, or a covering the body reliably forms on itself, frost among the examples given, so this record now sits inside the definition rather than beside it. `species`: "covering themselves in a near-impenetrable armor", and the record's signature, Mantle of Unyielding Winter, is a `hide` ward that layers frost over the pelt until blows stop reaching it. The forming is reliable rather than occasional: `physiology.composition.secondary` is `mineral`, and the ice is drawn from thin air by every individual, which is why this sits at 100 rather than at the 85 of iteration three. |
| `territorial` | The behavior the species exists for, in its own sentence rather than the planet's. `species`: "the Yetimoths formed the rank and file of Krystos' prisonguards in ancient times", and the next clause is about keeping prisoners in line, blocking off escape routes in walls of frost, and encapsulating whoever tries to pass. Holding ground it has been set to hold and denying passage rather than pursuing is the registry definition word for word. The Krystos Generator Environmental Report records guard-pattern forms retained from penal commissions among its fauna observations, which agrees from the other side. |

**Rolled (shares sum to 100)**

| Trait | Share | Evidence |
|---|---|---|
| `menacing` | 30 | A species sentence about what the creature is for, `species`: "to keep prisoners in line". A guard whose function is deterrence. Rolled rather than required because deterrence is downstream of the guard posting that `territorial` already carries. |
| `pack-bonded` | 27 | A species sentence, `species`: "the Yetimoths formed the rank and file of Krystos' prisonguards in ancient times". Rank and file is a body that works in formation with others. `solitary` is out of the pool, so the exclusion pair holds with one member. |
| `anchored` | 23 | A species sentence, `species`: "blocking off escape routes in walls of frost". A body that makes itself the barrier, on a world whose data block gives a gravity of 1.2 times Earth, carried on a frame of up to 470 kg. Below the two above it because nothing says it cannot be moved, only that it blocks. |
| `hardened` | 20 | The planet-wide adaptation, and the only rolled entry not carried by a species sentence. Sustained lethal cold is the first hazard entry in the Krystos Generator Environmental Report and cold-proof insulation is the first of its output priorities. The record already carries the fact as a temperature band of minus 60 to minus 10. Demoted from 100: the description is about the ice this creature makes, not about the cold it endures, so the world demands this of everything alive there rather than of this creature in particular. |
