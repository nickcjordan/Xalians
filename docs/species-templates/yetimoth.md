...
### Trait pool (expected count 3.97 before the 2026-09-07 evidence bar, 3.10 after it, 2.50 after iteration two, 5.30 after iteration three)
...
- `hardened` 100 : demanded species-wide by iteration three. Sustained lethal cold is the first hazard entry in the Krystos Generator Environmental Report and cold-proof insulation is the first of its output priorities, both of which name the physical-extremes trait. The record already carries the fact as a temperature band of minus 60 to minus 10; this is the body that band rests on. New entry, and the honest home of the cold argument that iteration two cut from `resistant`.
- `territorial` 100 : demanded species-wide by iteration three. The same report records guard-pattern forms retained from penal commissions among its fauna observations, with no quantifier, and this species is that form by its own description, `species`: "the Yetimoths formed the rank and file of Krystos' prisonguards in ancient times". Holding ground and denying passage rather than pursuing is what the whole description shows it doing. New entry.
- `dormant` 100 : demanded species-wide by iteration three. Metabolic suspension is an output priority of the Krystos Generator, which names the trait for what that unit prints. Note the line call in the report below: the fauna observation of metabolic-suspension forms is a form category this species does not obviously belong to, and the entry rests on the output priority alone. New entry.
...
Expected trait count after iteration three: (100 + 100 + 100 + 85 + 55 + 45 + 45) / 100 = 5.30, with no exclusion correction, since `solitary` is gone. `armored` at 85, `menacing` at 55, `anchored` at 45 and `pack-bonded` at 45 all sit below 100, so the variance requirement holds. The pool is well above the 3.5 guideline and is reported to the orchestrator rather than trimmed: the three new entries are each demanded by a named source, and a demanded trait cannot be hidden behind a percent.

`armored` re-checked and left at 85: the demanded-trait rule names chitin, plating, crystal, shell or metal armor, and this body's covering is fur with a mineral secondary composition, while the near-impenetrable armor of the description is ice it forms rather than a fixed surface. Nothing raises it, and it is the largest of the sub-100 entries.
