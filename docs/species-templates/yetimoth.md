...
- **feeding**: It eats what the frozen tundra gives, meat or otherwise.
...
Fields required (Nick, 2026-09-10): filled feeding ("It eats what the frozen tundra gives, meat or otherwise."). Source reading: the teaser, the diet key and the planet history; where they are silent the phrase is the plainest reading of the record and is flagged for Nick with the batch.
...
Pool shape, 2026-09-10 re-run: the lore split of 2026-09-09 and the five-field ratification of 2026-09-10 (`lore-status.json` marks `appearance` and `fields` `ratified` for Yetimoth) put `lore.appearance` and the five short fields on the same footing as the teaser, the art, and the planet record as pool sources (skill section 6, procedure step 9). Two required traits and four rolled entries whose shares sum to 100, expected count 3.00, unchanged from the 2026-09-08 shape; this is the one species in the batch that keeps two required traits, and the reasoning is set out below.

**Required**

| Trait | Evidence |
|---|---|
| `armored` | The act the description and the signature ability are both built around, and the case the widened definition names. Under the 2026-09-08 registry the key covers natural plating, shell, an integrated chassis, or a covering the body reliably forms on itself, frost among the examples given, so this record now sits inside the definition rather than beside it. `species`: "covering themselves in a near-impenetrable armor", and the record's signature, Mantle of Unyielding Winter, is a `hide` ward that layers frost over the pelt until blows stop reaching it. The forming is reliable rather than occasional: `physiology.composition.secondary` is `mineral`, and the ice is drawn from thin air by every individual, which is why this sits at 100. Reinforced by the ratified `lore.behavior` field: "It forms thick sheets of ice from thin air, armoring itself, walling off escape routes in frost, or encasing an opponent until it can lumber close enough to pummel it with its ice-gauntleted fists." |
| `territorial` | The behavior the species exists for, in its own sentence rather than the planet's. `species`: "the Yetimoths formed the rank and file of Krystos' prisonguards in ancient times", and the next clause is about keeping prisoners in line, blocking off escape routes in walls of frost, and encapsulating whoever tries to pass. Holding ground it has been set to hold and denying passage rather than pursuing is the registry definition word for word. The Krystos Generator Environmental Report records guard-pattern forms retained from penal commissions among its fauna observations, which agrees from the other side. Now further anchored in the species' own ratified fields: `lore.origin`, "Formed the rank and file of Krystos's prison guards in ancient times", and `lore.habitat`, "The high-security prison complexes of Krystos's frozen tundra and the waste outside them." |

**Rolled (shares sum to 100)**

| Trait | Share | Evidence |
|---|---|---|
| `pack-bonded` | 35 | Raised from a species-sentence inference to a directly ratified fact: `species`: "the Yetimoths formed the rank and file of Krystos' prisonguards in ancient times" already read as formation work, and the ratified `lore.company` field now states it outright: "It holds a line with others beside it." `solitary` is out of the pool and this field states the opposite of it, so the exclusion pair holds with one member. |
| `menacing` | 25 | A species sentence about what the creature is for, `species`: "to keep prisoners in line". A guard whose function is deterrence. Rolled rather than required because deterrence is downstream of the guard posting that `territorial` already carries; no ratified field adds further evidence of deterrence as this species' own act, so the share is trimmed to make room for the stronger `pack-bonded` entry. |
| `anchored` | 20 | A species sentence, `species`: "blocking off escape routes in walls of frost", echoed by the ratified `lore.behavior` field's "walling off escape routes in frost". A body that makes itself the barrier, on a world whose data block gives a gravity of 1.2 times Earth, carried on a frame of up to 470 kg. Below the two above it because nothing says it cannot be moved, only that it blocks. |
| `hardened` | 20 | The planet-wide adaptation, and the only rolled entry not carried by a species sentence. Sustained lethal cold is the first hazard entry in the Krystos Generator Environmental Report and cold-proof insulation is the first of its output priorities. The record already carries the fact as a temperature band of minus 60 to minus 10, and the ratified `lore.feeding` field, "It eats what the frozen tundra gives, meat or otherwise.", places the species itself in that cold rather than only the planet in general. Demoted from 100: the description is about the ice this creature makes, not about the cold it endures, so the world demands this of everything alive there rather than of this creature in particular. |

Why two required here and one everywhere else in this batch: the body itself demands nothing, since the covering is fur and the mineral composition is secondary, so neither required entry is a body fact in the ordinary sense. What the description gives instead is two distinct things in the same breath, an armor the creature makes and a post it is set to hold, and either one alone leaves half the creature unstated. `armored` without `territorial` is a bodyguard with no reason to stand anywhere; `territorial` without `armored` is a guard with no answer to being attacked. Both stay at 100 and the pool holds four rolled entries beneath them.

Traits considered and left out, with reasons (re-checked against the ratified `lore.appearance` list and the five fields; none of the checks below changed): `protective` (the ratified `lore.behavior` field is about encasing opponents and arming itself, never about shielding an ally); `ramming` (the ratified `lore.behavior` field's own word is "lumber", and the sprint band stays 20 to 35); `mind-sealed` (no field speaks to mental resistance); `solitary` (the exclusion partner; the ratified `lore.company` field, "It holds a line with others beside it.", states the opposite); `perceptive` (the senses bands top out at 75 and `physiology.senses.special` is absent, and no field adds a sourced perception claim); `resistant` (its only support was cold, already carried by `hardened`, and no field names a toxin, corrosion, radiation, or hostile-atmosphere hazard); `dormant` (its only support was the Krystos metabolic-suspension output priority, a planet-wide line with no species tie, and none of the fields describe waiting out a season); `inspiring` (the ratified `lore.company` field supports a body that holds a formation, not one that lifts the others).
...
- `pack-bonded`: 27 to 35. The ratified `lore.company` field, "It holds a line with others beside it.", is a direct species-level statement that did not exist at the prior pass; the earlier entry rested only on an inference from the teaser's "rank and file" clause. Raised to reflect the stronger, more direct sourcing.
- `menacing`: 30 to 25, trimmed only to keep the rolled total at 100 after `pack-bonded`'s increase; no new field changes the underlying evidence.
- `anchored`: 23 to 20, trimmed for the same reason; now additionally echoed by the ratified `lore.behavior` field's "walling off escape routes in frost".
- `hardened` unchanged at 20, now additionally echoed by the ratified `lore.feeding` field.
- `armored` and `territorial` unchanged at 100 each, both now additionally echoed by the ratified `lore.behavior`, `lore.origin`, and `lore.habitat` fields.
- No trait added or removed: nothing in the ratified fields introduces a fact this pool had not already captured, unlike Akinza's `lore.company` field, which introduced `solitary` outright.
...
| `perceptive` | 20 | newly cut 2026-09-08 under iteration three ruling 1: its only support was the graded bands `physiology.senses.hearing` [55, 75] and `smell` [55, 75], and a graded band now justifies the trait only where its upper bound reaches 80. Both stop at 75, and `physiology.senses.special` is absent, so nothing carries it |
...
| `dormant` | 100 | newly cut 2026-09-08 under the pool shape: its only support was metabolic suspension among the Krystos output priorities, and an output priority alone never brings a trait into a pool. The fauna observation of metabolic-suspension forms names a form category this species does not belong to, and the description shows a guard on duty rather than a body waiting out a season |
Expected trait count after the pool shape: (100 + 100 + 30 + 27 + 23 + 20) / 100 = 3.00, down from the 5.30 of iteration three, and the highest in this batch because it is the only species carrying two required traits. The chance an individual lands none of the four rolled traits is 0.70 times 0.73 times 0.77 times 0.80, which is 31.5 percent. Pool size is six entries, exactly at the cap, which is why `dormant` is cut above rather than kept at a small share.

`armored` raised from 85 to 100 as a required trait: the 15 percent that lacked it were variance on the frost-forming aspect, and the frost-forming aspect is the species. The registry key now names a covering the body reliably forms on itself alongside plating, shell and an integrated chassis, so making the armor rather than growing it is inside the definition and nothing about this entry is a stretched fit.

No `insulated`: the report's insulated high-mass tunneling forms line uses the word for thermal insulation on a world with no charge hazard at all, and the burrow band here is 15 to 30, which is not a tunneling form. The 2026-09-08 registry puts charge and static discharge in `insulated` and cold in `hardened`, so the Krystos fact lands on `hardened` alone. No `resistant` either: Krystos hazards are cold, blizzard cycles, avalanche and the preservation of organic material where it falls, with no toxin, corrosion, radiation, pathogen or chemically hostile air among them.

Friction resolved, 2026-09-08: the earlier pass reported that `armored` named only fixed surfaces a body is born with, and that the Yetimoth, which reaches the armored state by drawing sheets of ice out of the air and layering them over a fur pelt, therefore strained the definition. Nick ruled on the case the same day and widened the key to cover a covering the body reliably forms on itself, naming frost, resin and slag. The strain is gone: this record is now the definition's first example rather than its exception, and the entry stands at 100 on the widened reading.
...
Restored by the 2026-09-10 lore re-run: none of the traits cut above come back. `protective`, `ramming`, `mind-sealed`, `perceptive`, `resistant`, and `dormant` were each re-checked against the ratified `lore.appearance` list and the five fields (see the Traits considered and left out paragraph above), and none of the five fields adds the missing evidence class for any of them; the ratified `lore.behavior` field in particular restates the same armor-and-containment act the walkthrough already used to cut `protective` and `ramming`, rather than adding a new fact. `solitary` also stays cut, now on stronger grounds than before: the ratified `lore.company` field, "It holds a line with others beside it.", states the opposite of it directly rather than merely failing to mention it. This is the one difference from Akinza's re-run, where the equivalent company field introduced a trait that had been cut for lack of exactly that evidence; here the fields already ratified strengthen the traits already in the pool (`pack-bonded`, `armored`, `territorial`) rather than opening a new one.
...
Final run, after the 2026-09-10 trait re-run, from the worktree root: `node docs/species-templates/tools/validate-template.js yetimoth`
...
The two `conduits.source` WARNs are answered in the Instruments and conduits section above and in the WARN answers below (unchanged by this trait re-run: both conduits and their sentences are untouched). `temperature.planet` is a pre-existing infrastructure gap (the committed `planetRecords.json` carries no `environment.habitableBandC`, tracked as issue #167 per the validator script's own comment), not something this run's trait pool changes touch. The earlier `traits.expected` WARN no longer appears because the expected count is unchanged at 3.00, below the script's 3.5 threshold; it remains answered historically in the WARN answers section below.
...
- 2026-09-10, trait re-run (ratified lore fields now a pool source): raised `pack-bonded` 27 to 35 on the ratified `lore.company` field, "It holds a line with others beside it."; trimmed `menacing` 30 to 25 and `anchored` 23 to 20 to keep the rolled total at 100; `hardened` unchanged at 20; `armored` and `territorial` unchanged at 100 each; no trait added or cut, since no ratified field introduced a fact the pool had not already captured; expected count unchanged at 3.00.
