...
| Trait | Evidence |
|---|---|
| `armored` | The act the description and the signature ability are both built around. `species`: "covering themselves in a near-impenetrable armor", and the record's signature, Mantle of Unyielding Winter, is a `hide` ward that layers frost over the pelt until blows stop reaching it. The body agrees as far as it can: `physiology.composition.secondary` is `mineral`, and the ice is drawn from thin air by every individual, which is why this sits at 100 rather than at the 85 of iteration three. |
...
`armored` raised from 85 to 100 as a required trait: the 15 percent that lacked it were variance on the frost-forming aspect, and the frost-forming aspect is the species. The registry key names natural plating, shell or an integrated chassis, which this creature reaches by making the plating rather than growing it, and the strain in that fit is reported under Friction below rather than hidden behind a percent.
...
Friction reported, 2026-09-08: the registry defines `armored` as natural plating, shell or an integrated chassis, all of which are fixed surfaces a body is born with. The Yetimoth is the case where a creature reaches the armored state by an act, drawing sheets of ice out of the air and layering them over a fur pelt, and the record has to choose between a required trait whose definition it strains and a rolled percent that hides the one thing the description spends its longest clause on. The pool takes the first. The smallest fix would widen the `armored` definition to cover a covering the body reliably forms as well as one it is born with, which would also serve any future species that plates itself in stone, resin or slag; the case is recorded rather than worked around, and the entry stands at 100 until Nick rules.
