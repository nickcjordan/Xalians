...
- `regenerative 100`: raised to 100 by iteration three. A legacy rating that names a trait puts it at 100, and `recoveryRating` for Xylum is high; Floria's Generator Environmental Report agrees, listing regenerative tissue among its output priorities, which is a second naming source. The iteration-two reasoning, at the former 72, follows: restored 2026-09-07 under class 5, Nick's authored legacy design intent. The `statRatings` block for Xylum in `species.json` carries `recoveryRating` of high, which the widened bar maps directly to this trait. Floria's Generator Environmental Report lists regenerative tissue among its output priorities, which agrees, but the legacy rating alone is what carries the entry. Restored at the former 72.
- `perceptive 100`: raised to 100 by iteration three, since `physiology.senses.special` carries `tremorsense` and a special sense names this trait as a species-wide fact. Ruling 1 does not touch it: the graded bands are irrelevant where a special sense is listed, which matters here because the sight band tops out at 15. The iteration-two reasoning, at the former 15, follows: restored 2026-09-07 under class 4, a ratified field of the record. `physiology.senses.special` carries `tremorsense`, which the widened bar names outright as justification for this trait. On a blind buried body with a sight band topping out at 15, tremorsense is the only way it finds anything, and it stays at the authored 15.
- `territorial 100`: demanded species-wide. The Floria Generator Environmental Report states, without a quantifier, that the planet's territorial strategy is growth, entanglement, and regrowth in place of engagement. That is this species exactly: a rooted mass that holds the ground it occupies and denies passage rather than pursuing anything. New entry.
- `resistant 100`: raised to 100 by iteration three. Toxic fungal blooms are a hazard entry for Floria, and toxins are inside the narrowed scope of the trait, so it is the environmental adaptation every native must carry rather than a rolled weight. The iteration-one reasoning, at the former 35, follows: an environmental adaptation the planet may justify. The Floria history names "massive infestations of toxic fungi" (planet) in the same ground layer this species is buried in, so a soil-dwelling body that shrugs off contamination is a fair environmental weight, and environmental adaptation is the one thing a planet-wide sentence may support.
...
Expected trait count after iteration three: (100 + 100 + 100 + 100 + 100) / 100 = 5.00.

This pool is now entirely at 100, and there is no sub-100 entry left. That is reported rather than fixed, as iteration three directs. Every one of the five entries is demanded by a named source: `anchored` by the body, `regenerative` by the legacy `recoveryRating` of high and by Floria's regenerative-tissue output priority, `resistant` by the toxic-fungal-bloom hazard, `perceptive` by the `tremorsense` special sense, and `territorial` by the unquantified territorial-strategy line in the Generator report. No entry can be lowered without hiding a demanded trait, and no rolled entry survives the bar to sit beneath them: `healing`, `protective`, `solitary` and `menacing` all remain cut on the evidence they failed in iteration one. Every Xylum therefore carries the same five traits and individuals differ only in attributes, archetype, secondary element and signature intensity. The validator raises a WARN on this and no FAIL.

`resistant` re-checked under iteration three ruling 3 and kept at the raised 100: its support is the toxic fungal load of Floria's soil layer, which the Generator report repeats as a hazard. That is contamination, not thin atmosphere, dust, static discharge or temperature, so the narrowed definition does not touch it.
