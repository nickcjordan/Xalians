...
## September 26: scouting reading stability and desktop structure

Nick's live feedback identified a remaining priority 1 problem: scouting cards expanded and collapsed between text reveals, previous steps disappeared, and desktop viewing felt like an enlarged phone layout. This directly contradicts treating the earlier technical checks as proof that the interaction felt continuous. The preceding work shipped in PR #662; this entry records the follow-up.

- Removed timed progression from scouting and physical-return accounts. Readers advance at their own pace or reveal the full account, then explicitly leave it. Previously revealed text stays in an ordered, scrollable account rather than being replaced or divided into disappearing phone passages. Unrevealed findings remain undisclosed.
- Scouting map highlights no longer resize columns, change panel heights, insert temporary headings, or dim the resource display. The map still communicates actual scout movement and report delivery.
- Desktop uses a centered composition with a wider reading column and a smaller map aligned beside its top. Content width is bounded at 1,280 pixels rather than stretching two equal panels across the monitor. The account begins at the top and controls stay anchored. Phones retain a fixed map above the independently scrollable account.
- Browser checks at 1,600 by 1,000 and 2,519 by 1,411 covered the desktop composition. Measured map, account, first paragraph, and footer bounds were identical before and after report delivery at 1,600 pixels. At 390 by 667, map/account/footer bounds likewise stayed identical across all three reveals. At 320 by 568, the return account and its completion control remained readable and reachable.
- Checked both Graviclaw's remote report and Chromocat's physical return. Earlier paragraphs remain available by scrolling; no map state or creature cost was changed. Existing crossing and encounter playback remain separate from this scouting-specific change.
- Added retention, future-content concealment, manual progression, final handoff, and keyboard-focus coverage. All 85 targeted tests passed. Broader verification is in progress.
- Type checking and production build passed on the current main baseline. Under concurrent machine load, the full suite exceeded its five-second limit in three complete-expedition tests; those scenarios now have a fifteen-second ceiling with all outcome assertions retained. The separate portrait-load timeout passed in isolation and in the reduced-concurrency rerun. PR #671 contains this follow-up; final CI and deployment verification follow below when complete.
- Final verification: GitHub's frontend job passed all 132 test files and the production build on the final branch. The final local full run passed 1,740 of 1,742 tests; two other repeat-experience scenarios exceeded their existing five-second timeout under host load. No further timeout changes were made because the complete final CI suite passed. This local run is not recorded as a full pass.
- PR #671 merged as `8a6903eb3a02bd4700528317b7d6ffec06ef4291`. Production deployment run `36261127733` completed successfully. Reloaded `https://xalians.com/long-return`, resumed the annex-entry checkpoint, sent Graviclaw, and verified the published manual account and all three retained entries. The live desktop screenshot showed the wider reading column, smaller top-aligned map, and anchored completion controls. No browser console errors were reported.
- This reported scouting-layout follow-up is implemented, verified, and deployed. It does not establish that every phase of the game has ideal pacing or that the broader adventure catalog is complete. No executable fix remains from this specific scouting geometry and disappearing-text report; future subjective feedback remains evidence to act on, not presumed endorsement.

## September 26: restore automatic scouting storytelling

Nick rejected requiring clicks to reveal each event. The stable layout and retained account remain useful, but manual-only progression interrupted the story. Authorized outcome: scouting and return events unfold automatically with readable timing, earlier entries stay available, and the completed account remains until the player chooses the next gameplay action. Pause/resume and full reveal are optional controls. Verify automatic playback, pause, return reports, retained text, and stable desktop/phone geometry, then deploy through the repository workflow.

Implemented automatic text-length timing, replaced Next event with Pause/Resume story, and retained the final explicit gameplay handoff. Appended events scroll smoothly when needed, respecting reduced-motion preferences. Next unfinished action: run regression checks and verify actual playback in the browser before deployment.

Browser verification: the three-event Chromocat scouting account completed without any reveal clicks at 1600 by 1000. The physical return paused correctly, stayed on its first event while paused, and completed automatically after resume at 390 by 667. Account bounds remained x=10, y=235.59375, width=360, height=421.40625 before and after the appended return event. Type checking passed. Regression suite and production CI remain in progress.

The first local regression run passed the 14 focused story tests but found a keyboard-focus regression in manual full reveal, alongside 14 five-second expedition timeouts under concurrent host load. Fixed manual reveal to focus its final action explicitly while automatic completion leaves reading focus alone. Added coverage for retained reading focus during automatic playback. Full CI is required before release; local timeouts are not recorded as passing checks.
