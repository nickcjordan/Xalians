...
**One-cell-per-name STRUCK (Nick, 2026-09-07).** The rule that a name lives in exactly one action cell and one element file was never Nick's and is withdrawn: a name may sit in more than one cell, and in the neutral pool and an element cell, wherever each placement is logically sound on its own. Overlap is accepted as the price of exhaustive cells; the definition audit still governs each placement individually (Undertow still cannot be tagged dark).
