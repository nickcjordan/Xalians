...
Status: complete as of 2026-09-10. Every finding landed through PRs #152 to #201, including the page-level part of F2 and F20 (#195, the record view); see `backend-modernization-plan.md` for the wave-by-wave record. Kept as the audit of record; line references and the `lambda/` and `my-app/` paths describe the pre-migration tree (now `apps/api` and `apps/web`).
