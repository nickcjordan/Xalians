...
After the functional body is credible, make a separate character pass. Choose a small number of memorable, mutually supporting features such as a distinctive jaw or tail silhouette, one bold color boundary, or an expressive facial feature. Stylization may exceed a literal real-world animal model when the feature does not hinder the creature's documented movement, senses, feeding, or abilities. Do not make every flourish justify a new organ or power. Conversely, do not scatter ornamental marks without a coherent placement rule. Judge the whole creature at portrait and token sizes, and record any newly approved defining shape in the species description before treating it as canon. A generated image remains a proposal until that design choice is accepted.
