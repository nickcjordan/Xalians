...
| 13 | Immersive pages (duel board, playground, Reclamation, training games, Long Return) migrate in this program too, one brief each, so Bootstrap can leave the package. Their version 4 immersive design is a separate later concern; the migration reproduces their current look on the new stack. | 80% — Nick said replace it all; immersive redesign is still not started by his ruling | Nick 2026-09-09 |
...
3. **Immersive pages** (three briefs). Duel board and playground. Reclamation. Training games and the Long Return. Same look, new stack; these do not get their version 4 redesign here.
4. **Teardown** (one PR, by hand). Unlink and delete Bootstrap, the template, the three icon fonts, `system.css`, `style.css`, `tokens.css`, `typeColors.css`, the page CSS folder, the vendor folder; remove `bootstrap` and `react-bootstrap` from the package; turn on Tailwind preflight; rewrite the build-ui skill, the design contract's "where things live" section and `CLAUDE.md`. Paint check every route at 1440 and 390.
