...
4. **Teardown** (multiple measured PRs, by hand). Isolate the version 3 `.g-*` rules the immersive pages still read behind `src/styles/legacy/immersive.css` (with the `--g-*` tokens they need), then shrink and delete the old template and compatibility files as each selector is proven dead or route-owned; remove temporary Tailwind interop once no legacy specificity requires it. Paint check every route at 1440 and 390, the immersive pages included.

Each phase merges to `main` on its own so the site stays deployable throughout.

**Status, 2026-09-11:** phases 1 through 3 landed in PRs #139 to #148. Bootstrap, React-Bootstrap, the BootstrapMade template script, the three icon fonts and the vendor folder are gone from the package and page, and Tailwind preflight is on. Phase 4 is now a measured sequence: route-owned Duel and Reclamation sheets moved first; then the remaining compatibility cascade moved behind lazy immersive entries. Two interop rules stay while `src/styles/legacy/` survives: Tailwind utilities remain important because the legacy files carry hundreds of `!important` declarations, and bare anchors inherit their ink because the old template paints them mint. Both leave when the remaining selector audit deletes the old template layer.
