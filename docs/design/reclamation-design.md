...
| 24 | Hold is read off an instrument dial on a fixed scale: a half-round face printed 0 to 20 with a brass over-range sector to 30 for home ground, a bone needle at the reading, a dim second needle for what the site takes, the number beside it. The face is neutral (no side color) so the dial carries only the quantity; the ground says the side and the chip's rim says the world | 80% (Nick, 2026-09-04: keep a consistent scale so the max is always known, and the thin blue or amber strip was hard to read and over-committed to the two side colors; the dial is my answer) | `reclamationFigure.HoldMeter`, `reclamation.css` `.rec-gauge*` |
