...
| 12 | The story is a click-through viewer: one beat at a time in a fixed box, moved on only by the reader (Next and Back, the chapter bar, the arrow keys while it is on screen, a horizontal swipe); the page scrolls past it like any section. A beat opens still, the picture arriving first, and goes live once its entrance has settled (1.15 s); only the shown beat can be live, and nothing is live while the box is less than half on screen, mid-change, or in a hidden tab. On the last beat Next becomes Read on, down to the Galaxy section. On the way past, the viewer rests centered in the screen for about a third of a screen of scroll, then moves on with the page (Nick 2026-09-26: "a slight pause in the middle of the screen and then it's a tiny bit sticky when you start to scroll again"): native CSS sticky over an in-flow spacer, no scroll listener, and the story itself never moves with the scroll. A window too short for the box shows the shown beat in the page at its own height. (Nick 2026-09-24: "it's going to be hard to support a site where we allow scrolling with an active animation... I'm leaning towards the alternative... a click-through presentation.") | 90%, Nick's lean, built for him to judge live | `pages/home/storyViewer.tsx`, `scripts/plates/snap-story.cjs` |
| 13 | Every beat plays back as a recording from an archive, on an old playback screen: scan lines, grain, a slow rolling band, and a readout (Standby, Tuning or Playback, a running clock, and "Rec NN" with where it was recorded: Floria, Grimedes, Genome record, Valleron). Until the viewer rests (held centered by its pause, or on a short screen with its picture fully in view, for 0.22 s) the screen stands by, dark, with nothing heavy in it; then it tunes in (a burst of static, the picture opening out of a bright line, 1.1 s) and plays. Scrolling on switches it off (the picture collapses to a line, 0.38 s). Choosing another beat cuts through a burst of static. The readout is interface-plain and makes no lore claim beyond the place. (Nick 2026-09-26: theme the scenes "as if they were video files stored in some digital archive... a recording from when the researchers had been on the planet", with the screen coming to life through static, so nothing heavy plays while scrolling.) | 90%, Nick's direction, built for him to judge live | `pages/home/archiveScreen.tsx`, `.archive` in `globals.css`, `--color-archive-*` in `tokens.css` |
| 14 | The recordings play at a film rate, 20 frames a second, and the living plates are baked: their still parts are pictures, only what moves is SVG. At the screen's rate the Unbirth plate kept the main thread and the graphics chip saturated, which is what made the whole page jumpy around it; at 20 the page around it is smooth, and the lower rate suits archive footage. (Nick 2026-09-26: "it just feels super jumpy... it needs to be smoother"; he chose the film rate and the bake over video to try first.) | 85%, the frame rate is a look Nick judges live | `components/plates/livePlate.tsx` (`FILM_FPS`), `scripts/plates/bake-plate.cjs`, `docs/design/living-plate-playbook.md` |
