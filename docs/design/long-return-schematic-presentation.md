...
- In the Simple decision view, the numbered key also carries crew energy and site stability. Do not repeat a separate crew-status strip above the map. Keep urgent universal conditions beside their creature; explain role restrictions at that role's choice and performance penalties beside the affected plan. Names stay intact on narrow screens. If nobody can scout, remove the impossible selection action and offer staying together directly.
...
- Route choice presents one scene introduction beside the schematic, rather than a repeated objective plus separate geography descriptions. The shared comparison retains its metric axis on phones. Route headings may stay visible while costs scroll, but the full diagram must not be pinned over the reading area. Keep the physical approach and arithmetic available in route analysis.
...
- At the Index exit, distinguish the haul banked by leaving now from the maximum additional haul across the named number of remaining optional crossings. Reading an offer is not a commitment. Keep explicit departure/continuation buttons, the actual current-haul risk, and optional settlement details; do not imply the next room alone promises the whole deeper reward.
...
- A lasting choice names its practical next-room effect in the comparison. On arrival, that earned effect changes the affected path's label and adds a small stationary marker on the path, not another notification beneath the diagram. The underlying event and calculation stay available in requested analysis. Unapplied or unearned effects must not change the map.
