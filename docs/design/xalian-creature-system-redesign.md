...
## Guiding principle: levers, not stone (Nick, 2026-09-02)

Every decision in this document and the artifacts it governs (registry definitions, trait model and percents, instrument matrix, conduit medium table, size and lifespan rules, catalog cells, signature rulings) is a **tuned lever**, ratified so work can proceed consistently, and expected to be fine-tuned as the platform is built. Every game and feature will consume real creatures, and each build is a test of the system. Agents building with creatures report friction in the moment, with the concrete case and the smallest fix, and Nick decides; adjustments land in the lever's source of truth (this document, `REGISTRY-DEFINITIONS.md`, `RULINGS.md`, the script `CHANGELOG.md`, the migrate-species skill) and affected records are re-run or amended with a dated note. Versioning (`generatorVersion`) means a lever can move without invalidating minted creatures. The full statement lives in the repo `CLAUDE.md`.
...
```jsonc
{
  ...
  "traits": ["armored", "anchored", "stealthy"],   // RATIFIED 2026-09-02: the keys that landed, nothing else; the percents live in the species template pinned by generatorVersion, so rarity is recomputable forever
...
**Archetype weights are percents (ratified 2026-09-02).** `archetypeWeights` lists a subset of the 16 archetypes as integer percents summing to exactly 100; one archetype is rolled per creature, so the number is the chance, on the same reading as trait percents. This replaced the 1 to 5 relative weights left over from the first Graviclaw draft; the nine records then finished were converted by largest remainder with order preserved.

**Conduits (mechanism and medium table ratified 2026-09-02).** The instrument-by-action matrix stays purely physical. A species may declare instruments as conduits for an element (`conduits: { instrument: element }`), only where the description or art shows the element's power leaving through that part, and only for the primary or an on-graph secondary. For an ability of that element, a conduit instrument's allowed actions are its physical row plus the element's medium row (a 14-row table: what each element can do when channeled). Two gates, both required: the medium matches and the part is a declared conduit. This replaces per-species signature exceptions for element-flavored delivery (Hypnopet's entrancing horn, Figzy's projected force, Graviclaw's gravity through the pincers). Full text and table in the migrate-species skill, section 5.7a.
...
```jsonc
{
  ...
  "archetypeWeights": { "skirmisher": 62, "vanguard": 25, "balanced": 13 },   // integer percents summing to exactly 100 (ratified 2026-09-02); one archetype rolled per creature
