...
## Data layer (agent A1, owns `apps/web/src/lore/`, `scripts/`, `docs/encyclopedia/narration.json` loading only, and the lore tests)

New `apps/web/src/lore/story.js`:
...
### Era plates (2026-09-07)

Each of the seven parts of The Story now opens with a painted era plate above the definition: a flat gouache frontispiece generated locally with Z-Image-Turbo, framed as a matte specimen plate with a short caption underneath. The manifest lives at `docs/encyclopedia/plates.json` (file names, alt text, caption, and the run/seed that produced each image, so a plate can be regenerated); the images themselves are under `apps/web/public/assets/img/lore/eras/`, a 1536x768 painting plus a 768x384 phone variant per era. `scripts/bundleLore.js` copies the manifest into the generated JSON bundle the same way it already handles `tour.json` and `narration.json`, and `story.js` attaches the resolved plate to each part so `Story.js` only renders what it is given. A new validator test, `lore.plates.test.js`, checks the manifest has exactly seven plates, one per real era, with every image file present on disk and captions free of dashes, four-digit years, and the banned-word list the narration validator already enforces.
