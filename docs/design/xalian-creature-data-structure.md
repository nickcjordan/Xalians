...
```jsonc
{
  ...
  "traits": ["armored", "anchored", "stealthy"],   // RATIFIED 2026-09-02: the landed keys, nothing else; each was rolled independently at the percent in the species template (listed traits 1 to 100, unlisted = 0) pinned by generatorVersion
...
- **Traits (24 generic keys)** with modes (passive|reactive), e.g. armored, anchored, stealthy, phasing, regenerative, healing, protective, pack-bonded, solitary, ramming, hypnotic, mind-sealed. Each is rolled independently at its own percent (1 to 100, unlisted = 0); a planet fact may justify an environmental adaptation trait at or near 100, behavioral traits are never planet-justified, and a species-fixed body fact (chitin, gravity-rooting) is simply a pool entry at 100. A ratified tilt system makes rolled bodies tilt trait odds (stealthy tilts small-mass, phasing tilts high ghost affinity, etc.) — honest causal correlates only.
- **Archetypes (16):** vanguard, juggernaut, berserker, bulwark, survivor, stalwart, skirmisher, runner, prowler, predator, seeker, sage, virtuoso, sovereign, rogue, balanced. Each SHAPED build favors exactly two attributes (skew-only, never punishing), balanced favors none, and every attribute is favored by exactly three builds.
- **The ability-name catalog** (complete): all 14 element catalogs plus the neutral pools are consolidated and bundled (`docs/ability-catalog/`, `scripts/bundleAbilityCatalog.js`), about 21,600 element names and 1,100 neutral names; names live in cells keyed by action × element medium, deliberately near-exhaustive so move names feel semi-unique across creatures (the Pokemon shared-move model is explicitly rejected). Every anatomy-evoking name carries instrument tags so generation can filter by the creature's actual body. Signature abilities use a grander register exempt from the 2-word catalog limit and are hand-authored per species (exactly one each, collision-checked, always the lore-defining act).
