...
The compiler partitions by delivery and whether/which first effect uses area. Independent categorical domains remain factored. It validates every supported field relationship at authoring time, checks source anatomy/voice/gaze across the full physiological bands, and proves enough distinct actions can be selected. At runtime it selects unused structure indices directly, removes equivalent representations across mechanisms, and then rolls output. It does not enumerate a species' entire move universe, evaluate lore per individual, or generate/reject/retry creatures.

The current validator's domain proof relies on the bounded schema: within a recipient partition, each variable constraint spans at most two dimensions. Future higher-order relationships require extending that proof and its tests. This is not a general predicate engine. Overlapping mechanisms preserve authored sampling weight; selection is not advertised as uniform over unique semantic structures. Effect alias matching can be combinatorially expensive for many indistinguishable effect entries; consolidate redundant authoring and benchmark representative canonical species before release.
