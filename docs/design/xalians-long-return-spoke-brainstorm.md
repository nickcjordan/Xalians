...
Resolution is deterministic once all relevant tags are known. Concealed tags create uncertainty, not invisible dice. A method can succeed while producing different costs: time, noise, strain, elemental exposure, lost salvage, or rising facility pressure. This is essential. “Break the door,” “phase through the door,” “decode the door,” and “pull its mechanism from the other side” should all work, but leave a different mission state.
...
- strain, pressure, one extraction point, and one optional deep objective;
...
The interface should separate **mission difficulty** from **information difficulty**. Guidance changes how clearly command interprets known data; it never changes a creature's scores, the route target, hidden information, strain, pressure, rewards, or consequences.
...
This is the first layer of a broader accessibility spectrum. A later Guided mode can recommend route and role assignments as well as methods; Standard can retain contextual definitions and historical comparisons; Expert can remove most predictive language. These modes should remain switchable during a mission so learning the system is not a permanent commitment made at setup.
