...
## Open rulings for Nick

1. **Does the Magmuth Massacre belong inside the Accords era, or should it remain part of the general Age of Generators company-wars narrative?** This draft places it in the Accords, immediately before APEX's turn, because Magmuth ¶6 fixes it "mere months before APEX would turn on the Vallerii race," which reads as tighter to the war's outbreak than to the general company-wars period. The recommended resolution is to keep it in the Accords era as drafted, since the anchor phrase is explicit and load-bearing.

2. **Does the Age of Unbirth overlap with the first wave of Generators, or should the era boundary be drawn earlier, at sterility alone?** Several worlds (notably Drainov and Stonera) explicitly frame their Generator installations as events "of" the Age of Unbirth, even though installing and running a Generator is functionally the start of the Generation era. This draft keeps the Unbirth era open through the first wave of Generators and closes it once Generators become common technology. The recommended resolution is to ratify this reading, since it matches how the source paragraphs describe themselves, but Nick should confirm whether he'd rather define Unbirth strictly as the sterility crisis alone, with all Generator installations moved into the Generation era.

3. **Are the "Imperial Houses" and the "Old Houses" the same governing body?** Phantiri's source uses both terms without ever equating them explicitly. This draft treats them as the same body for narrative continuity. The recommended resolution is to ratify this as a working assumption unless Nick wants to draft new lore distinguishing them, in which case this Chronicle would need a revision.

4. **Where does Phantiri's history sit relative to the Accords, End Wars, Source Code 606, and the Nemesis Plague, none of which are ever mentioned in its source text?** This draft places most of Phantiri's material in the Age of Generators and present day, treating its total silence on the intervening galaxy-wide events as a deliberate feature (the text explicitly states "APEX never once turned its attention" to the world) rather than a gap to be filled in. The recommended resolution is to preserve this silence rather than invent connective tissue Phantiri's own source never provides.

5. **Should minor spelling variants in verbatim quoted prose (such as "Poesidas" in Veridium ¶2) be corrected in this document?** This draft preserves them as quoted. The recommended resolution is to leave all such spellings uncorrected pending Nick's explicit sign-off on any change to the underlying JSON, consistent with the standing rule against silent corrections.

6. **Should the Chronicle remain permanently undated, or would relative markers (Era 1, Era 2, and so on) better serve future cross-referencing even without literal dates?** This draft uses named eras only, with no numbering, matching the way the source material itself refuses to fix any date. The recommended resolution is to keep it undated, since imposing numeric ordering risks implying a precision the canon does not support, but Nick may prefer numbered eras purely as a navigation aid.
