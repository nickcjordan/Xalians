...
- **Open, not planned:** sound synthesized in code and off by default, a guided first Proving. (A Loki line shipped in Pass 2, below.)

## Pass 2 levers (2026-09-06)

Three levers from Pass 1's friction and the digest's area-majority reading (Blood Rage's Loki line, the roster economy's spending-beats-saving tilt) were tuned and measured in the simulator. All three land in `expeditionRules.js`, `expeditionBot.js`, and `expeditionInterpretation.js`, with tests in the three `__tests__` files. `docs/design/reclamation-design.md` is updated wherever a rule changed.

**1. The hide rule.** The base rule now reads: hide when the send flips or contests a world the rival can still answer (the rival has not passed) and the creature's hold is at least the site's current margin; send openly when the send leaves the site solidly ahead. The old rule (`stealthy and |margin| < hold`) was the flip condition restated, because the bot's own scoring always prefers a flip or contest over securing an already-won site (`FLIP_VALUE` far outweighs `SECURE_VALUE`), so the picked candidate's pre-send margin was at most zero on effectively every real send; the old rule read "hide" on 96 percent of stealthy sends regardless of the board. The fix judges the site by where the send leaves it (the pre-send margin plus this creature's own hold) rather than where the site stood before the creature arrived. Before, hidden send rates sat at 27 to 29 percent for all five rivals, no matter their `hideBias`. After, over 200 matches against the proctor (seed 11): envoy 12.8 percent hidden, heir 14.3, broker 20.3 (`hideBias` 1.8), proctor 14.4 (`hideBias` 1, the baseline), windsailor 15.4. The broker's `hideBias` now visibly moves the rate above the proctor's, which is the surface it was meant to have. No new constant; the fix is entirely in how the existing `hideBias`, `applyHideBias`, and the base-rule computation in `chooseSend` read the board.

**2. The roster economy: the trailing seat's compensation.** Tested (a) `SENDABLE` 10 to 9, (b) the trailing seat's compensation as an extra send instead of moving first, and (c) doubling the last frame's worlds toward the clinch. (c) was not shipped, per the brief. Baseline ("no change," with lever 1 already in): envoy 45.5 percent against the proctor, heir 50.5, broker 44.0, windsailor 55.5 (200 matches, seed 11); starter win rate 51 percent; comeback rate after a lost first round 24 to 27 percent across rivals. Variant (a), `SENDABLE` 9: the spread widened rather than narrowed (envoy 49.5, windsailor 62.0, broker 41.5), and windsailor's comeback rate fell to 20.6 percent, on the edge of the floor; rejected. Variant (b), one bonus send for the trailing seat's next round only, replacing "trailing moves first" with plain alternation: over 400 matches at a different seed (23) for confirmation, the five rivals against the proctor came in at envoy 46.8, heir 45.5, broker 47.3, proctor 48.8, windsailor 36.5 percent, all but windsailor within a 3-point band and every one of the first four above 45 percent; starter bias held at 46 to 50 percent; comeback rates rose to 32 to 40 percent, well clear of the floor. Shipped as `ROSTER_TRAILING_BONUS = 1` (`expeditionInterpretation.js`), applied in `expeditionRules.js`'s `judge()` (which now alternates the starter unconditionally and computes `trailingBonus` per side) and `send()`/`hasLegalSend()` (via the new `sendableCapFor()` helper, `SENDABLE + trailingBonus[player]` for that round only); `getPublicState` exposes each side's own `sendableCap` so the bot (and the table) can read its actual cap without inferring it, since `sitesWon` was already public. The bot's `remainingSends` in `chooseSend` reads `me.sendableCap` instead of the bare constant, falling back to `SENDABLE` for any caller that predates the field.

**3. A Loki line.** Both variants were simulated: (A) a creature withdrawn from a LOST (not tied) world returns to the roster and may be sent again, but its next send costs `RETURNED_SEND_COST` (2) against the sendable cap; (B) the same return, but the creature arrives staggered (half hold) for that round instead of costing extra. Both kept match shape sane: routs per match 3.9 to 4.9 in both variants (today's baseline range), staggers 2.4 to 2.7 in both, third frames still commonly the closer (5-4 and 6-3 the most common final scores in both), and neither variant moved any rival's win rate against the uniformly random policy (all stayed 93 to 97 percent, same as before the lever). Variant A's win-rate spread across rivals (38.5 to 52.5 percent) was tighter than variant B's (31.5 to 50.5, with windsailor dropping hardest), and A is the simpler rule (it does not interact with the existing stagger/resilient/mend machinery the way a second, differently-caused stagger does), so A ships. Returned sends are rare and not exploitable: about 0.5 per match (2.5 to 2.8 percent of all sends) across every rival, winning their world 47.5 to 59.3 percent of the time versus 55.3 to 57.8 percent for a normal first send, a real but modest markdown for the double cost. Shipped as `RETURNED_SEND_COST = 2` (`expeditionInterpretation.js`). `judge()` splits the lost-vs-tied branches it used to fold together: a LOST world (the opponent won outright, not a tie) returns the withdrawn records to `roster` and adds their ids to a new `returned` array on player state; a TIED world still withdraws with no return, exactly as before. `send()` charges `RETURNED_SEND_COST` against the round's sendable cap for a flagged record (illegal if the remainder cannot afford it) and clears the flag once it is sent again; `sendableRoster()` filters out a returned record the cap cannot afford. `getPublicState` exposes `returned: [ids]` in the self view only (the opponent's roster contents, and therefore which of their records are returned, stay hidden, same as the roster itself). The bot skips a returned record it cannot afford, and otherwise prices it at its normal value minus the extra unit's hold cost, so it is not preferred over a fresh send at equal hold but is still sent when it is the best option left. `devtools/expeditionSimulator.js` gained `returnedSendStats` (rate, per-match count, site win rate returned versus first-send) in the combat section of the report, the same way `hiddenSendStats` is measured, so this lever stays visible to future tuning.
