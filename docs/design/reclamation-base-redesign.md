...
## Pass 4: hiding is concealment only, and the bot reads (2026-09-10)

Nick's brief: hiding had become "a concept that is getting a little too out of place" (a hidden send cost two, landed first, at three quarters power). Agreed: strip hiding to its information half. A stealthy creature may be sent hidden; the rival learns that something was sent, not which creature or where, until the Clash; the send costs one and the creature attacks in speed order at full power. The first-strike bonus is gone rather than moved, since speed already orders attacks and a second ordering rule was the original fault. Nick then asked why the bot could not use "opponent has a hidden creature" as a factor, and it can: the bot now reads unseen sends.

**The read.** Every hidden send the rival has made this round, plus the sends still to come from a rival who has not passed (anticipation), is assumed to be worth a fixed hold (`hiddenHoldGuess`) and spread across the round's worlds (`readSharpness`: 0 evenly, higher values by where the rival would most want a creature, priced with the bot's own flip and secure values off the visible board). A candidate send is valued as the average of its worth under each guess rather than against a single hedged margin: the first version subtracted the spread guess from every world's margin and measured WORSE than ignoring hidden sends, one to four points on three seeds, because the phantom half-creature tipped the flip thresholds the scoring is built on. A hidden send is one of the sends the handler was already anticipating, not an extra one (the unseen count is the larger of the two while the rival can still send, the hidden count alone once it has passed); adding them made the read's weight jump every time the rival hid, which measured as hiding helping or hurting the hider by five points depending only on the anticipation setting.

**What the read turned out to be.** Anticipation is the whole of it. A proctor that assumes one more rival send is coming beats the pass 3 proctor, which scored the board as if the rival would never move again, by 68 to 72 percent on three seeds at 1000 matches, and the handler that ignores unseen creatures altogether by 86 to 91. The guess about WHERE the rival will send adds nothing: the even spread beats the sharp read by 56 to 57 on every seed, so the shipped read is a hedge, not a hunch, and the sharpness knob stays as a rival's lever. This is a bot-strength change, not a rules change, and every gauge in the validation report is read against the stronger proctor from here on.

**Hiding at the bot's level.** Against a reading proctor, never hiding loses by about seven points on three seeds at 1000 matches, and the proctor already hides every stealthy creature it sends (the always-hidden policy is now bit-identical to the proctor: with hiding free, "hide when the world is still contested" is always true under anticipation). So at the bot's level hiding is not a choice at all, it is what a stealthy creature does. Whether a human ever sends a stealthy creature openly, and whether a human can read a hidden send better than an even spread, are the human session's questions; the simulator reports concealment's worth and no longer gauges it.

**Ladder.** The stronger proctor broke the envoy (7 to 9 percent against it), whose rationing doubled the price of hold as well as refusing overspend; with the default hold cost it reads 40 to 42 on three seeds. The broker hides everything now that hiding is free. Re-measured at 400 to 1000 matches: envoy 41, heir 47, proctor 48, broker 48.5, windsailor 50.5, and the intro's order follows.

| # | Assumption / Decision | Confidence | Supporting Evidence |
|---|---|---|---|
| 24 | Hiding is concealment only: costs one, lands in speed order at full power; `hiddenFirst`, `hiddenSendCost` and `hiddenPower` stay as ablation levers, off | 85% (Nick, 2026-09-10) | rulebook assumption 60 |
| 25 | The bot reads unseen sends by anticipation (one coming send while the rival has not passed, hidden sends counted within it), guess 8, spread evenly, valued by the average over the guesses | 80% (1000-match sweeps on seeds 7, 13, 21; `expeditionBot.js` HIDDEN_HOLD_GUESS, READ_SHARPNESS, ANTICIPATION) | Pass 4 measurements |
| 26 | Concealment's worth is reported, never gauged; the always-hidden regret row is reported only | 85% (against a bot it is worth only what the bot's guess is wrong by) | `expeditionValidation.js` section 8 |

### Pass 4 measurements (2026-09-10)

**Head to head, 1000 matches, seeds 7 / 13 / 21** (the variant's win rate against what it is compared with): proctor with anticipation against the pass 3 bot 71.9 / 70.6 / 68.2; against a blind bot (ignores hidden and coming sends alike) 86 to 91.5 at 200 matches; guess 8 against guess 4 66.6 / 65.5 / 62.8 (guess 10 63.8 / 64.2 / 61.7, guess 12 60.7 / 56.2 / 61.6, guess 6 53.6 / 55.7 / 54.1); even spread against the sharp read 55.7 / 56.8 / 56.7; anticipation 2 against 1 44.5 / 46.8 / 43.0. Never hiding against the proctor 39.6 / 43.1 / 41.3 against a mirror of 46.3 / 50.0 / 48.6; always hiding 46.5 / 50.0 / 48.4, bit-identical to the mirror. The sharp read against the even spread 42.7 / 40.6 / 39.5.

**Validation, seed 7, 200 matches** (`reclamation-validation-report.md`): proctor mirror 45.0 (seat A; 50.0 on seed 13, 49.5 on 21); greedy 0, always-stack 0, never-contest 0, random 3.5, pass-early 22.5, presence-first 24.0, always-hidden 45.5 (reported only). The naive gauge is met with room on every seed. Spread 3.0 near-best, dominant on 30 percent (the pass 3 bot read 5.8 and 12.6: the reading bot sees fewer ties between candidates, which is what a scorer that tells worlds apart should do). Decided after round 1 46 / 48 / 49 by seed; comeback 32.1 / 30.6 / 25.7 (the 30 to 40 band met on two seeds for the first time, without any gift); resolution changes the leader at 15.2 / 14.2 / 14.9 percent of contested worlds (band 25 to 40, unmet, and lower than pass 3's 17.8); **downs per match 2.06 / 1.88 / 1.71, under the 3 to 5 band for the first time** (pass 3 read 2.8 to 3.1): the reading bot spreads its sends across worlds the rival can still answer rather than stacking into fights, so fewer creatures meet. Stake taken in 30.5 percent of Provings, 81.5 percent by the trailing side, staked world held 51.6 against 49.6 unstaked, comeback 32.1 with against 33.0 without: still variance-neutral. Ablation against the new proctor: hidden sends, the Loki line, speed order, sweep, shield, hurt-attacks-less, willful, presence scale, the swift move and the stake all move something beyond the interval; hidden-first restored and the pass 3 prices restored both move the hidden rate and downs (the priced game cuts hiding to 5.6 percent and downs to 1.68); **the bolster role, bolster recovery and the instinct lanes move nothing measurable at 200 matches**, and the bolster role is a new entry on that list (it was the second-heaviest rule under the pass 3 bot). Draft unchanged in kind: dromeus dead, fire dead, no dominant species, roles on one bar (bolster 51.2, strike 50.8, shield 49.2, sweep 48.8).

**Lanes per role**, the reading Nick asked about: strength +11.8 overall (+9.4 within strikes, +17.1 within presences), resilience +12.7 (+18.4 within presences), endurance +9.0 (+19.0 within presences), vitality +7.0; intelligence -1.6 overall and -9.7 within presences; charisma -6.1 overall and **-15.2 within presences**, the role whose scale charisma sets; agility -6.2 overall and -16.3 within presences. So the two lanes Nick named are not flat, they are negative, and the presences carry the sign: a high-charisma or high-intelligence presence loses its world more often than a low one. The likeliest cause is the draft and the send, not the rule: `rateForDraft` and the bot's role value prize hold, so the presences that get kept and sent are the ones whose points went to hold rather than to charisma or intelligence, and the lane reads the trade-off. Next pass.
