...
| 4 | Each plate is composed natively for its panel's shape (Nick, 2026-09-22: the SVG rebuild is free of the raster's frame). The viewBox is always 1536 wide and as tall as the panel's aspect needs: Unbirth 21:9 is 1536 by 658, Accords 4:5 is 1536 by 1920, Present 4:3 is 1536 by 1152, Generation 16:9 is 1536 by 864. The frame's 6px mat makes the well wider than the panel's aspect (Unbirth: 2.37 on desktop, 2.45 on a phone), which matters for short panels: `meet` letterboxed Unbirth with the poster showing at both sides, so short panels use `slice` and keep important content out of the top and bottom 5%. | 90% | Nick's ruling in conversation; `pages/home.tsx` panel aspects; the capture tools scale by width |
| 5 | Only one living plate is ever live on a page (Nick, 2026-09-23). `plateStage.ts` gives the stage to the plate most in view; every other plate drops its SVG out of the DOM and shows its poster, a still of its own t=0 frame. Fragments are cached, so a plate coming back re-injects without a request. A page can pass `active` to decide for itself: the home story's click-through viewer (2026-09-24) does, and makes only its shown beat live, after its entrance has settled. | 95%, checked by scrolling the home page at desktop, phone and reduced motion: never more than one plate's SVG in the DOM | `components/plates/plateStage.ts`, `livePlate.tsx`, `livePlate.test.tsx`, `pages/home/storyViewer.tsx`, `scripts/plates/snap-story.cjs` |
| 6 | The site plays a baked plate at a film rate (2026-09-26). `bake-plate.cjs` turns every still piece into a picture (`live.html` plus `live/*.webp`, drawn at twice the plate's size) and simple masks into clip paths, and fails unless the baked plate matches the export pixel for pixel at several moments; `LivePlate` keeps every timeline paused and steps it forward `FILM_FPS` (20) times a second. Measured on the home page, same machine, same run, against the unbaked plate at the screen's rate: Unbirth went from about 40 frames a second with the main thread and graphics chip both saturated to about 140 with the main thread at 60%, End Wars from about 105 to about 230, and injecting Unbirth from about 90 ms to about 30 (about 550 to 200 at a four-times-slower processor). | 85%, the frame rate is a look Nick judges live | `scripts/plates/bake-plate.cjs`, `components/plates/livePlate.tsx` |
...
- What a playing plate costs is its moving layers: each change makes the browser redraw that layer where it changed, everything under the change in the same layer included (filters, masks, patterns), and resample and restyle every animated element on the main thread. Still layers cost nothing once drawn. So keep expensive still work out of moving layers (the bake turns it into pictures, but only where a run of still siblings sits outside any moving group), keep masks off moving groups where a clip would do, and keep the count of animated elements down: on Unbirth the glow layer's many small water effects (rain rings, splashes, whitecaps, about 670 animations) are about half of what the plate still costs after baking.
- The site plays `live.html`, never `plate.html`: run the bake after every export (section 9).
...
1. `python scripts/plates/export-plate.py <era>` writes `public/assets/plates/<era>/`, then `node scripts/plates/bake-plate.cjs <era>` (dev server on 3012) bakes it into `live.html` and `live/`, and checks it against the export by pixels. Never ship an export without its bake: the site plays the baked plate.
2. In `pages/home.tsx`, add `live: '/assets/plates/<era>/live.html'` to that era's entry in `ART`. The plate's viewBox already matches the panel, so no poster should show at any edge; if one does, the viewBox is wrong.
...
4. Run `node scripts/plates/snap-posters.cjs <era>` (dev server running) to capture the plate's poster, its own t=0 frame at 1536 and 768 wide, and set `still` on its `ART` entry. The poster is what shows whenever another plate holds the stage, so recapture it every time the plate changes.
5. Publish the source to the review artifact if one exists, open the PR from a fresh branch off main, and enable auto-merge.
6. Update the page's text in the source (the kicker line and notes) whenever the plate changes; the End Wars page carried a stale "pass" label for many rounds.
