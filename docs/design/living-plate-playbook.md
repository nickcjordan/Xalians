...
| 5 | Only one living plate is ever live on a page (Nick, 2026-09-23). `plateStage.ts` gives the stage to the plate most in view; every other plate drops its SVG out of the DOM and shows its poster, a still of its own t=0 frame. Fragments are cached, so a plate coming back re-injects without a request. A page can pass `active` to decide for itself; the home story no longer does (its pinned stage was replaced by the page's own scroll on 2026-09-24), and its small pieces join the same stage, so only one thing on the page animates. | 95%, checked by scrolling the home page at desktop, phone and reduced motion: never more than one plate's SVG in the DOM | `components/plates/plateStage.ts`, `livePlate.tsx`, `livePlate.test.tsx`, `pages/home/helixPiece.tsx`, `scripts/plates/snap-story.cjs` |
