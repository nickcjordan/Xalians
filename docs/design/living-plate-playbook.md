...
| 5 | Only one living plate is ever live on a page (Nick, 2026-09-23). `plateStage.ts` gives the stage to the plate most in view; every other plate drops its SVG out of the DOM and shows its poster, a still of its own t=0 frame. Fragments are cached, so a plate coming back re-injects without a request. A page that shows its plates one scene at a time (the home story's stage, 2026-09-23) passes `active` instead and decides itself: the outgoing plate plays while it fades, the incoming shows its poster and goes live once its entrance settles. | 95%, checked by scrolling the home page at desktop, phone and reduced motion: never more than one plate's SVG in the DOM | `components/plates/plateStage.ts`, `livePlate.tsx`, `livePlate.test.tsx`, `pages/home/storyStage.tsx`, `scripts/plates/snap-stage.cjs` |
