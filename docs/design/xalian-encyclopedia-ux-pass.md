...
| 4 | Trail and read marks are per-browser (localStorage), never synced and never sent anywhere. The lore layer stays pure; the marks live in a UI utility. | 95% | `apps/web/src/lore/index.js` header comment |
| 5 | Powers: the three demonym entries currently categorized `factions` (magmuthites, the-zolto, veridians) are shown only under Xalian Peoples, not Factions; `vallerii` and `king-kozrak` render under a section titled "The Vallerii". Categories in `encyclopedia.json` are left as they are (keys and categories are append-only per the internal doc); the page groups them. | 85% | `apps/web/src/lore/entries.js` getPowers, `encyclopedia.json` categories |
...
## Data layer additions (`apps/web/src/lore/`)
...
The trail utility is `apps/web/src/components/encyclopedia/trail.js` (written by the orchestrator before fan-out): `recordVisit({ kind, key, name, element })`, `getTrail()`, `clearTrail()`, `markRead(kind, key)`, `isRead(kind, key)`, `useTrail()` hook returning `[trail, clear]`, `useVisit({ kind, key, name, element })` hook that records the visit and marks read on mount and returns whether it was already read, `useReadMark(kind, key)` read-only hook that re-renders when marks change. Kinds: `entry`, `world`, `species`, `era`, `chapter` (key `<planet>:<index>`), `beat`.
...
Nick's asks: one source location per kind of data, the continuous Chronicle reader, the coverage sweep, connections, and site wiring (no contrast or screen-reader pass yet). Shipped: species entries live only in `docs/encyclopedia/encyclopedia.json` (the per-species `.encyclopedia.json` side files were byte-identical and are gone; `speciesRecords.json` carries records only; the template validator looks the entry up by key); an optional `aliases` field on entries drives the linker (first occurrence per paragraph), search, connections, and the coverage script; `/encyclopedia/read` and `/read/:era` (the seven parts as one book, margin notes, sticky progress rail, read marks shared with the world pages); Connections on entries, worlds, and species ranked by `count * log(totalUnits / documentFrequency)` so galaxy-wide records fall below specific ones; `scripts/loreCoverage.js` writing `docs/encyclopedia/COVERAGE.md` (after aliases the real gaps are "5th Armada" and "Frackworm"; four entries nothing links to); the generator, account, duel roster, duel squad picker, and home carousel link into the archive through `components/encyclopediaLink.js`, and the home page carries an Encyclopedia Xalia section. Hash-only navigation now scrolls to its target. Spelling: "Aglael" corrected in Poseidas.
