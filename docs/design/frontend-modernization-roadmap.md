...
- First selector-audit local verification: typecheck, 52 files / 1,027 tests, zero production vulnerabilities, production build, and all tightened budgets pass. Desktop/mobile smoke covers Training, Match, Physics, Duel setup/reference, a started 64-cell Duel board, Reclamation, and Long Return without page/console errors or new overflow; representative before/after paints are unchanged. PR/CI/deploy/production: pending.
