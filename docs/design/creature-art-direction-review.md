...
**Third pass (2026-09-21).** Nick ruled the silhouette portraits a reference only, with the stage look open. The rig library gained render styles (`plain`, `toon`, `flat`, `ink`) as a spec-level lever, and every spec-built species now plays in each style on `art/creature-motion-comparison/styles.html`. Observed, not decided: toon and flat both replace the plastic register with an illustrated one from the same specs, toon reads best on Dromeus and Avilily at phone size, flat is nearest the pilot cutout, and the ink homage is not reachable from a render setting. The contour also exposes template faults that the plain render hid. The 2D ceiling test (hand-drawn replacement cels on the Akinza cutout) is the next half of the comparison.
