...
**Open:** The final motion and visual style, camera/view policy, and clip requirements. Moving comparisons and production effort now exist for Avilily (two Blender passes beside four other methods) and for one challenging body plan (Bioflim in metaballs). Nick has not reviewed the second pass. The next evidence should be a toon or outlined render of the 3D source against the plain render, a bloom fix checked at phone size, and the swarm body (Vespersyn).
