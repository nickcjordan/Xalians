...
- The repository-wide web build previously failed when any initial, vendor or route JavaScript/CSS graph exceeded a fixed raw or gzip number. That rule came from frontend modernization, not a browser or hosting constraint. At the owner's direction, the budget file and checker are removed from the web build. Vite still reports emitted assets, the production styleguide remains excluded, and lazy route boundaries remain test-covered. The content package's separate `check:bundle` verifies generated data integrity rather than asset size and is unaffected.
- Audited Long Return history for cuts made specifically to satisfy that gate. The most consequential was the latest trapped-native encounter: the companion's moment of hesitation and choice to join, the detour's physical retreat, the rig's unresolved door, the forceful escape, and the annex strain during rescue had all been compressed. Those beats are restored while retaining the new marked-scout causal thread. A spent scout's retrieval account again gives the crew's journey and structural delay room to read. An older accessibility-label cut that omitted the “known” status from mapped hazards is also restored. Three icon mappings removed in an earlier compact-scout pass were unused; restoring dead mappings would not help the player. Historical budget measurements below describe the former gate, not a current product requirement.
- Verification: 1,346 web tests, typecheck, and an uncapped production build passed. Two rebuilt 390×667 three-room replays covered the marked/no-relay Hypnopet path (37 clicks) and aid/physical-return path (29 clicks); the marked scout and prepared-crew choice screens were reviewed. The Xylum aid record again includes the sustained rescue and resulting annex strain. The temporarily allied creature and spent-scout edge still depend on targeted tests rather than these specific browser paths. Human evaluation of narrative quality remains open.

### September 18 checkpoint: a marked scout approach now carries through contact and return

- A three-room phone replay exposed a causal gap at the trapped Hypnopet: the scout action was called “Mark the safe controls,” but its account only described withdrawal, and the later crew encounter did not credit the marks for avoiding surprise. The scout now visibly marks the rig controls, the contact choice previews the safer approach, and the returning report and crew response connect that preparation to the same physical rig. The native remains trapped until the crew decides what to do; marking it does not clear the path. A no-relay scout's encounter context also shows the possible return delay and resource costs before choosing a response.
- A separate edge-case audit found that a scout already at zero energy was described as walking back and offered a “Wait for return” action. The crew now retrieves that scout, the schematic brings both parties together, and the cue/report show only resources actually lost. This does not revive a spent creature or change the existing return cost rules.
- Verified the marked/no-relay path against a rebuilt 390×667 local production preview through three scenes and 37 clicks, including the physical return and prepared group encounter. Phone scout and crew choice screens were paint-reviewed. Regression tests cover marked report timing, group preparation, scout retrieval map positions and zero-energy narration. All 1,345 web tests, typecheck, build and bundle budgets passed. The spent-scout edge was unit-tested, not browser-replayed; human assessment of whether the causal link reads at a glance remains open. This is a natural checkpoint, not completion of the larger seven-scene UX goal.
...
- A second local replay exposed a subtler contradiction: the account said support removed an exhausting push even though an unseen charge and cold still cost the lead four energy. That no-exhaustion line now appears only when the resolved lead actually spent none; otherwise the physical hazard and environmental causes stand on their own. All seven scenes of the 390-pixel natural deep-retrieval path were replayed after the revisions with the same 17-salvage, two-stability outcome. All 1,339 web tests, typecheck, production build and unchanged bundle budgets passed. Route-wide and targeted tests guard the changed wording; no resource rule, result, map position or advanced calculation changed. Human evaluation of the prose remains open.
