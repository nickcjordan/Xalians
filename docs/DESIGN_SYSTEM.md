...
| The home story's stage: the four story spreads shown one at a time on a pinned stage; moving to the next spread fades the shown painting out the way the page travels (exit eases in, 560 to 640 ms) while the next drifts in after it (enter eases out, 700 to 950 ms, delayed 180 ms), then its caption slides out from its side (delayed 620 ms); reversed when scrolling back, instant under reduced motion | CSS transitions keyed off each scene's state | `pages/home/storyStage.tsx`, `.story-scene` in `globals.css` |
