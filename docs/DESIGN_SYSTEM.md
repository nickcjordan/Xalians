...
| The home story in the page's own scroll: its beats one after another, each beat's words (and a small piece) rising in once as they enter (`[data-reveal]`, opacity and 24px, 800 ms ease-out, never visibility, so unrevealed words stay in the accessibility tree); a sticky chapter bar whose dot is the middle of the screen, written to a custom property each animation frame and following under reduced motion because it is feedback; the small pieces loop on their own clock only while they hold the page's plate stage | GSAP ScrollTrigger `once` for the reveal; the dot is `--story-progress`; pieces use `loopAt` | `pages/home/storyFlow.tsx`, `pages/home/helixPiece.tsx`, `pages/home/motion.ts`, `.story-line` in `globals.css` |
