...
| The home story's click-through viewer: one beat at a time, moved on only by the reader (Next and Back, chapter bar, arrow keys, swipe). Next sends the shown frame out the way the story travels (exit eases in, 560 to 640 ms) while the next drifts in after it (enter eases out, 700 to 950 ms, delayed 180 ms), then its reading column (delayed 420 ms) and its label (delayed 720 ms) arrive; Back plays it the other way. The chapter bar's dot glides to the shown beat (520 ms). A beat goes live only after its entrance settles (1.15 s); one instant cut under reduced motion | CSS transitions keyed off each beat's state; the dot is `--story-progress` | `pages/home/storyViewer.tsx`, `.story-scene`, `.scene-spread` and `.story-line` in `globals.css` |
| The home story's archive screen (lore art): standby until the viewer rests, then tuning (static burst, the picture opening out of a bright line, 1.1 s), playback (a slow rolling band, a blinking lamp, the clock ticking once a second), a static cut between recordings (0.52 s), and off (collapse to a line, 0.38 s) when the reader scrolls on; under reduced motion every change is a cut and nothing loops | CSS keyframes keyed off `data-screen` | `pages/home/archiveScreen.tsx`, `.archive` in `globals.css` |
| A living plate (the End Wars painting on the home story): fire, smoke, weapon fire, water and lamps in motion inside the painting itself. The one ruled exception to "nothing loops" (Nick, 2026-09-22): it is the picture, not the interface. It plays only while in view and the tab is shown, and holds its resting frame under reduced motion; a page shows one at a time. It plays at a film rate of 20 frames a second, and its still parts are baked pictures | SMIL inside stacked SVG layers, stepped by hand | `components/plates/livePlate.tsx`, baked fragment at `public/assets/plates/<era>/live.html` (`scripts/plates/bake-plate.cjs`) |
