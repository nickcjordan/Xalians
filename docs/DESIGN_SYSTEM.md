...
| The home story's stage: the four story scenes shown one at a time on a pinned stage, with a timeline whose dot follows the scroll position directly (every scroll moves it; written to a custom property each animation frame, and it keeps following under reduced motion because it is feedback). When the dot reaches the next scene's marker, the shown painting fades out the way the page travels (exit eases in, 560 to 640 ms) while the next drifts in after it (enter eases out, 700 to 950 ms, delayed 180 ms), then its reading column (delayed 420 ms) and its label (delayed 720 ms) arrive; reversed when scrolling back, one instant cut under reduced motion | CSS transitions keyed off each scene's state; the dot is `--story-progress` | `pages/home/storyStage.tsx`, `.story-scene` and `.story-line` in `globals.css` |
